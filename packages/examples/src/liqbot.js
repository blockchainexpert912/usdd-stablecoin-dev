...
const { Decimal, UserTrove, USDD_LIQUIDATION_RESERVE } = require("@liquity/lib-base");
...
async function tryToLiquidate(liquity) {
  ...
  try {
    ...
    const expectedCompensation = total.collateral
      ...
      .add(USDD_LIQUIDATION_RESERVE.mul(troves.length));
    ...
    const { collateralGasCompensation, usddGasCompensation, liquidatedAddresses } = receipt.details;
    ...
    const totalCompensation = collateralGasCompensation
      .mul(store.state.price)
      .add(usddGasCompensation);

    success(
      `Received ${bold(`${collateralGasCompensation.toString(4)} ETH`)} + ` +
        `${bold(`${usddGasCompensation.toString(2)} USDD`)} compensation (` +
