...
contract("TroveManager - Redistribution reward calculations", async accounts => {
  ...
  it("redistribution: A, B Open. B Liquidated. C, D Open. D Liquidated. Distributes correct rewards", async () => {
    ...
    const alice_Coll = (await troveManager.Troves(alice))[1]
      .add(await troveManager.getPendingCollReward(alice))
      .toString();
    const carol_Coll = (await troveManager.Troves(carol))[1]
      .add(await troveManager.getPendingCollReward(carol))
      .toString();
    ...
    */
    ...
    const entireSystemColl = (await activePool.getColl(collToken.address))
      .add(await defaultPool.getColl(collToken.address))
      .toString();
    assert.equal(
      entireSystemColl,
    ...
    // check USDD gas compensation
    assert.equal((await usddToken.balanceOf(owner)).toString(), dec(400, 18));
  });
  ...
  it("redistribution: A, B, C Open. C Liquidated. D, E, F Open. F Liquidated. Distributes correct rewards", async () => {
    ...
    const bob_Coll = (await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingCollReward(bob))
      .toString();
    const dennis_Coll = (await troveManager.Troves(dennis))[1]
      .add(await troveManager.getPendingCollReward(dennis))
      .toString();
    const erin_Coll = (await troveManager.Troves(erin))[1]
      .add(await troveManager.getPendingCollReward(erin))
      .toString();
  ...
  it("redistribution: Sequence of alternate opening/liquidation: final surviving trove has ETH from all previously liquidated troves", async () => {
    ...
    const freddy_ETHReward = (await troveManager.getPendingCollReward(freddy)).toString();
    ...
    */
    ...
    // check USDD gas compensation
    assert.equal((await usddToken.balanceOf(owner)).toString(), dec(1000, 18));
  });
  ...
  it("redistribution: A,B,C,D open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
    ...
    const denominatorColl_1 = (await troveManager.getEntireSystemColl(collToken.address)).sub(
      A_entireColl_0
    );
    ...
    const B_ETHGain_1 = await troveManager.getPendingCollReward(B);
    const C_ETHGain_1 = await troveManager.getPendingCollReward(C);
    const D_ETHGain_1 = await troveManager.getPendingCollReward(D);
    const E_ETHGain_1 = await troveManager.getPendingCollReward(E);
    ...
    const denominatorColl_2 = (await troveManager.getEntireSystemColl(collToken.address)).sub(
      C_entireColl_1
    );
    ...
    const B_ETHGain_2 = await troveManager.getPendingCollReward(B);
    const D_ETHGain_2 = await troveManager.getPendingCollReward(D);
    const E_ETHGain_2 = await troveManager.getPendingCollReward(E);
    ...
    const denominatorColl_3 = (await troveManager.getEntireSystemColl(collToken.address)).sub(
      E_entireColl_2
    );
    ...
    const B_ETHGain_3 = await troveManager.getPendingCollReward(B);
    const D_ETHGain_3 = await troveManager.getPendingCollReward(D);
  ...
  it("redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    await addColl({
      collAmount: addedColl,
      upperHint: bob,
      lowerHint: bob,
      extraParams: { from: bob }
    });

    // Alice withdraws USDD
    ...
    // Expect Bob now holds all Ether and USDDDebt in the system: 2 + 0.4975+0.4975*0.995+0.995 Ether and 110*3 USDD (10 each for gas compensation)
    const bob_Coll = (await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingCollReward(bob))
      .toString();

    const bob_USDDDebt = (await troveManager.Troves(bob))[0]
      .add(await troveManager.getPendingUSDDDebtReward(bob))
      .toString();
    ...
    assert.isAtMost(
      th.getDifference(bob_USDDDebt, A_totalDebt.mul(toBN(2)).add(B_totalDebt).add(C_totalDebt)),
      1000
    );
  });
  ...
  it("redistribution: A,B,C Open. Liq(C). B tops up coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    /* Bob rewards:
     L1: 1/2*0.995 ETH, 55 USDD
     L2: (2.4975/3.995)*0.995 = 0.622 ETH , 110*(2.4975/3.995)= 68.77 USDDDebt

    coll: 3.1195 ETH
    debt: 233.77 USDDDebt

     Alice rewards:
    L1 1/2*0.995 ETH, 55 USDD
    L2 (1.4975/3.995)*0.995 = 0.3730 ETH, 110*(1.4975/3.995) = 41.23 USDDDebt

    coll: 1.8705 ETH
    debt: 146.23 USDDDebt

    totalColl: 4.99 ETH
    totalDebt 380 USDD (includes 50 each for gas compensation)
    */
    const bob_Coll = (await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingCollReward(bob))
      .toString();
    ...
    const alice_USDDDebt = (await troveManager.Troves(alice))[0]
      .add(await troveManager.getPendingUSDDDebtReward(alice))
      .toString();
    ...
    assert.isAtMost(th.getDifference(bob_Coll, expected_B_coll), 1000);
    assert.isAtMost(th.getDifference(bob_USDDDebt, expected_B_debt), 10000);
    ...
    assert.isAtMost(th.getDifference(alice_USDDDebt, expected_A_debt), 10000);
  ...
  it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). C tops up. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    const alice_ETHReward_1 = await troveManager.getPendingCollReward(alice);
    const bob_ETHReward_1 = await troveManager.getPendingCollReward(bob);
    const carol_ETHReward_1 = await troveManager.getPendingCollReward(carol);

    //Expect 1000 + 1000*0.995 ETH in system now
    const entireSystemColl_1 = (await activePool.getColl(collToken.address))
      .add(await defaultPool.getColl(collToken.address))
      .toString();
    assert.equal(
      entireSystemColl_1,
    ...
    const entireSystemColl_2 = (await activePool.getColl(collToken.address)).add(
      await defaultPool.getColl(collToken.address)
    );
    th.assertIsApproximatelyEqual(
      entireSystemColl_2,
    ...
    */
    ...
    const entireSystemColl_3 = (await activePool.getColl(collToken.address))
      .add(await defaultPool.getColl(collToken.address))
      .toString();
    th.assertIsApproximatelyEqual(
      entireSystemColl_3,
    ...
    // check USDD gas compensation
    th.assertIsApproximatelyEqual((await usddToken.balanceOf(owner)).toString(), dec(400, 18));
  });
  ...
  it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). A, B, C top up. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    const entireSystemColl_2 = (await activePool.getColl(collToken.address))
      .add(await defaultPool.getColl(collToken.address))
      .toString();
    th.assertIsApproximatelyEqual(
      entireSystemColl_2,
    ...
    */
    ...
    const entireSystemColl_3 = (await activePool.getColl(collToken.address)).add(
      await defaultPool.getColl(collToken.address)
    );
    th.assertIsApproximatelyEqual(
      entireSystemColl_3,
  ...
  it("redistribution: A,B,C Open. Liq(C). B withdraws coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    // Expect Bob now holds all Ether and USDDDebt in the system: 2.5 Ether and 300 USDD
  ...
  it("redistribution: A,B,C Open. Liq(C). B withdraws coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    /* Bob rewards:
     L1: 0.4975 ETH, 55 USDD
     L2: (0.9975/2.495)*0.995 = 0.3978 ETH , 110*(0.9975/2.495)= 43.98 USDDDebt

    coll: (1 + 0.4975 - 0.5 + 0.3968) = 1.3953 ETH
    debt: (110 + 55 + 43.98 = 208.98 USDDDebt 

     Alice rewards:
    L1 0.4975, 55 USDD
    L2 (1.4975/2.495)*0.995 = 0.5972 ETH, 110*(1.4975/2.495) = 66.022 USDDDebt

    coll: (1 + 0.4975 + 0.5972) = 2.0947 ETH
    debt: (50 + 55 + 66.022) = 171.022 USDD Debt

    totalColl: 3.49 ETH
    totalDebt 380 USDD (Includes 50 in each trove for gas compensation)
    */
    const bob_Coll = (await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingCollReward(bob))
      .toString();
    ...
    const entireSystemColl = (await activePool.getColl(collToken.address)).add(
      await defaultPool.getColl(collToken.address)
    );
    th.assertIsApproximatelyEqual(
      entireSystemColl,
    ...
    const entireSystemDebt = (await activePool.getUSDDDebt()).add(await defaultPool.getUSDDDebt());
    th.assertIsApproximatelyEqual(
      entireSystemDebt,
  ...
  it("redistribution: Trove with the majority stake withdraws. A,B,C,D open. Liq(D). C withdraws some coll. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    //Expect 1995 ETH in system now
    const entireSystemColl_1 = (await activePool.getColl(collToken.address)).add(
      await defaultPool.getColl(collToken.address)
    );
    th.assertIsApproximatelyEqual(
      entireSystemColl_1,
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Distributes correct rewards", async () => {
    ...
    const C_pendingRewardsAfterL1 = th
      ...
      .div(B_coll.add(C_coll));
    assert.isAtMost(
      th.getDifference(await troveManager.getPendingCollReward(bob), B_pendingRewardsAfterL1),
      1000000
    );
    assert.isAtMost(
      th.getDifference(await troveManager.getPendingCollReward(carol), C_pendingRewardsAfterL1),
      1000000
    );
    ...
    assert.isAtMost(
      th.getDifference(await troveManager.getPendingCollReward(carol), C_pendingRewardsAfterL2),
      1000000
    );
    assert.isAtMost(
      th.getDifference(await troveManager.getPendingCollReward(dennis), D_pendingRewardsAfterL2),
      1000000
    );
    ...
    const carol_pendingETHReward = (await troveManager.getPendingCollReward(carol)).toString();

    const dennis_rawColl = (await troveManager.Troves(dennis))[1].toString();
    const dennis_pendingETHReward = (await troveManager.getPendingCollReward(dennis)).toString();

    const erin_rawColl = (await troveManager.Troves(erin))[1].toString();
    const erin_pendingETHReward = (await troveManager.getPendingCollReward(erin)).toString();
    ...
    const activeColl = (await activePool.getColl(collToken.address)).toString();
    const defaultColl = (await defaultPool.getColl(collToken.address)).toString();
    ...
    // check USDD gas compensation
    assert.equal((await usddToken.balanceOf(owner)).toString(), dec(600, 18));
  });
