...
contract("HintHelpers", async accounts => {
  ...
  /* Open a Trove for each account. USDD debt is 200 USDD each, with collateral beginning at
  1.5 ether, and rising by 0.01 ether per Trove.  Hence, the ICR of account (i + 1) is always 1% greater than the ICR of account i. 
 */

  // Open Troves in parallel, then withdraw USDD in parallel
  const makeTrovesInParallel = async (accounts, n) => {
    ...
    const withdrawUSDDpromises = activeAccounts.map(account => withdrawUSDDfromTrove(account));
    await Promise.all(withdrawUSDDpromises);
  ...
  const withdrawUSDDfromTrove = async account => {
    await borrowerOperations.withdrawUSDD(th._100pct, "100000000000000000000", account, account, {
      from: account
    });
  };

  // Sequentially add coll and withdraw USDD, 1 account at a time
