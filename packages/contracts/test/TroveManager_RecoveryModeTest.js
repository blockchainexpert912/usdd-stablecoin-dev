...
contract("TroveManager - in Recovery Mode", async accounts => {
  ...
  beforeEach(async () => {
    ...
    const communityIssuanceTester = DEFTContracts.communityIssuance;
  ...
  it("checkRecoveryMode(): Returns true if TCR falls below CCR", async () => {
    ...
    // price drops to 1ETH:150USDD, reducing TCR below 150%.  setPrice() calls checkTCRAndSetRecoveryMode() internally.
  ...
  it("liquidate(), with ICR < 100%: updates system snapshots correctly", async () => {
    ...
    // --- TEST ---
    // price drops to 1ETH:100USDD, reducing TCR below 150%, and all Troves below 100% ICR
    await priceFeed.setPrice("100000000000000000000");
  ...
  it("liquidate(), with 100 < ICR < 110%: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    //  Bob withdraws up to 2000 USDD of debt, bringing his ICR to 210%
    ...
    // --- TEST ---
    // price drops to 1ETH:100USDD, reducing TCR to 117%
    await priceFeed.setPrice("100000000000000000000");
  ...
  it("liquidate(), with 100% < ICR < 110%: updates system snapshots correctly", async () => {
    ...
    //  Bob withdraws up to 20000 USDD of debt, bringing his ICR to 210%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(150, 16)),
      extraParams: { from: alice }
    });
  ...
  it("liquidate(), with 100% < ICR < 110%: offsets as much debt as possible with the Stability Pool, then redistributes the remainder coll and debt", async () => {
    ...
    // Alice deposits 390USDD to the Stability Pool
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: alice });
    ...
    // check pool USDD before liquidation
    const stabilityPoolUSDD_Before = (await stabilityPool.getTotalUSDDDeposits()).toString();
    assert.equal(stabilityPoolUSDD_Before, "390000000000000000000");
    ...
    /* Now, liquidate Bob. Liquidated coll is 21 ether, and liquidated debt is 2000 USDD.
    
    With 390 USDD in the StabilityPool, 390 USDD should be offset with the pool, leaving 0 in the pool.
  
    Stability Pool rewards for alice should be:
    USDDLoss: 390USDD
    ETHGain: (390 / 2000) * 21*0.995 = 4.074525 ether

    After offsetting 390 USDD and 4.074525 ether, the remainders - 1610 USDD and 16.820475 ether - should be redistributed to all active Troves.
   */
    // Liquidate Bob
    await troveManager.liquidate(bob, { from: owner });

    const aliceDeposit = await stabilityPool.getCompoundedUSDDDeposit(alice);
    ...
    /* Now, check redistribution to active Troves. Remainders of 1610 USDD and 16.82 ether are distributed.
    ...
    L_USDDDebt = 1610 / 6 = 268.333 USDD
    L_ETH = 16.820475 /6 =  2.8034125 ether
    */
    const L_USDDDebt = (await troveManager.L_USDDDebt()).toString();
    const L_ETH = (await troveManager.L_COLL()).toString();
  ...
  // --- liquidate(), applied to trove with ICR > 110% that has the lowest ICR, and Stability Pool USDD is GREATER THAN liquidated debt ---

  it("liquidate(), with 110% < ICR < TCR, and StabilityPool USDD > debt to liquidate: offsets the trove entirely with the pool", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 USDD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 USDD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(240, 16)),
      extraUSDDAmount: dec(250, 18),
      extraParams: { from: bob }
    });
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
    ...
    // Alice deposits USDD in the Stability Pool
    ...
    /* Check accrued Stability Pool rewards after. Total Pool deposits was 1490 USDD, Alice sole depositor.
    As liquidated debt (250 USDD) was completely offset

    Alice's expected compounded deposit: (1490 - 250) = 1240USDD
    Alice's expected ETH gain:  Bob's liquidated capped coll (minus gas comp), 2.75*0.995 ether
  
    */
    const aliceExpectedDeposit = await stabilityPool.getCompoundedUSDDDeposit(alice);
    const aliceExpectedETHGain = await stabilityPool.getDepositorCollGain(alice);
    ...
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getCollateral(bob, collToken.address),
      bob_remainingCollateral
    );
    // can claim collateral
    const bob_balanceBefore = th.toBN(await collToken.balanceOf(bob));
    await borrowerOperations.claimCollateral({ from: bob, gasPrice: 0 });
    const bob_balanceAfter = th.toBN(await collToken.balanceOf(bob));
    th.assertIsApproximatelyEqual(
      bob_balanceAfter,
  ...
  it("liquidate(), with ICR% = 110 < TCR, and StabilityPool USDD > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 USDD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 USDD of debt, resulting in ICR of 220%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(220, 16)),
      extraUSDDAmount: dec(250, 18),
      extraParams: { from: bob }
    ...
    */
    ...
    // check Bob’s collateral surplus
    th.assertIsApproximatelyEqual(await collSurplusPool.getCollateral(bob, collToken.address), "0");
  });

  it("liquidate(), with  110% < ICR < TCR, and StabilityPool USDD > debt to liquidate: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 USDD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 USDD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool USDD > debt to liquidate: updates system snapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 USDD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 USDD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool USDD > debt to liquidate: closes the Trove", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 USDD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 USDD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool USDD > debt to liquidate: can liquidate troves out of order", async () => {
    // taking out 1000 USDD, CR of 200%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: alice }
    });
    ...
    await openTrove({
      ICR: toBN(dec(200, 16)),
      extraUSDDAmount: totalLiquidatedDebt,
    ...
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getCollateral(dennis, collToken.address),
    ...
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getCollateral(carol, collToken.address),
      carol_remainingCollateral
    );

    // can claim collateral
    const dennis_balanceBefore = th.toBN(await collToken.balanceOf(dennis));
    await borrowerOperations.claimCollateral({ from: dennis, gasPrice: 0 });
    const dennis_balanceAfter = th.toBN(await collToken.balanceOf(dennis));
    assert.isTrue(
    ...
    const carol_balanceBefore = th.toBN(await collToken.balanceOf(carol));
    await borrowerOperations.claimCollateral({ from: carol, gasPrice: 0 });
    const carol_balanceAfter = th.toBN(await collToken.balanceOf(carol));
    th.assertIsApproximatelyEqual(
      carol_balanceAfter,
  ...
  USDD is LESS THAN the liquidated debt: a non fullfilled liquidation --- */

  it("liquidate(), with ICR > 110%, and StabilityPool USDD < liquidated debt: Trove remains active", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 USDD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 USDD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
      extraUSDDAmount: dec(1500, 18),
      extraParams: { from: alice }
    });
    const { collateral: B_coll } = await openTrove({
      ICR: toBN(dec(240, 16)),
    ...
    // Alice deposits 1490 USDD in the Stability Pool
    ...
    /* Since the pool only contains 100 USDD, and Bob's pre-liquidation debt was 250 USDD,
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool USDD < liquidated debt: Trove remains in TroveOwners array", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 USDD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 USDD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
    ...
    // Alice deposits 100 USDD in the Stability Pool
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool USDD < liquidated debt: nothing happens", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 USDD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 USDD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
    ...
    /*  Since Bob's debt (250 USDD) is larger than all USDD in the Stability Pool, Liquidation won’t happen
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool USDD < liquidated debt: updates system shapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 USDD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 USDD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool USDD < liquidated debt: causes correct Pool offset and ETH gain, and doesn't redistribute to active troves", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 USDD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 USDD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({
    ...
    const L_USDDDebt_After = (await troveManager.L_USDDDebt()).toString();
    const L_ETH_After = (await troveManager.L_COLL()).toString();

    assert.equal(L_USDDDebt_After, "0");
    assert.equal(L_ETH_After, "0");
  });

  it("liquidate(), with ICR > 110%, and StabilityPool USDD < liquidated debt: ICR of non liquidated trove does not change", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 USDD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 USDD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    // Carol withdraws up to debt of 240 USDD, -> ICR of 250%.
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
      extraUSDDAmount: dec(1500, 18),
      extraParams: { from: alice }
    ...
    //Check SP USDD has been completely emptied
    assert.equal((await stabilityPool.getTotalUSDDDeposits()).toString(), dec(100, 18));
    ...
    await usddToken.transfer(bob, dec(100, 18), { from: alice });
    ...
    // Alice provides another 50 USDD to pool
    ...
    //Check SP USDD has been completely emptied
    assert.equal((await stabilityPool.getTotalUSDDDeposits()).toString(), dec(150, 18));
  ...
  it("liquidate() with ICR > 110%, and StabilityPool USDD < liquidated debt: total liquidated coll and debt is correct", async () => {
    // Whale provides 50 USDD to the SP
    await openTrove({
      ICR: toBN(dec(300, 16)),
      extraUSDDAmount: dec(50, 18),
      extraParams: { from: whale }
    });
    ...
    const entireSystemCollAfter = await troveManager.getEntireSystemColl(collToken.address);
  ...
  it("liquidate(): Doesn't liquidate undercollateralized trove if it is the only trove in the system", async () => {
    // Alice creates a single trove with 0.62 ETH and a debt of 62 USDD, and provides 10 USDD to SP
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    /*  Since there is 0 USDD in the stability Pool, A, with ICR >110%, should stay active.
  ...
  it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
    ...
    //Dennis provides 200 USDD to SP
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({
      ICR: toBN(dec(220, 16)),
      extraUSDDAmount: dec(1000, 18),
      extraParams: { from: whale }
    });

    const { usddAmount: A_usddAmount } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraUSDDAmount: dec(300, 18),
      extraParams: { from: alice }
    });
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, re-open, be reedemed and claim again", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 USDD of debt, resulting in ICRs of 266%.
    // Bob withdraws up to 480 USDD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(240, 16)),
      extraUSDDAmount: dec(480, 18),
      extraParams: { from: bob }
    });
    const { collateral: A_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
    ...
    // Bob re-opens the trove, price 200, total debt 80 USDD, ICR = 120% (lowest one)
    ...
    // can claim collateral
    const bob_balanceBefore_2 = th.toBN(await collToken.balanceOf(bob));
    await borrowerOperations.claimCollateral({ from: bob, gasPrice: 0 });
    const bob_balanceAfter_2 = th.toBN(await collToken.balanceOf(bob));
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, after another claim from a redemption", async () => {
    // --- SETUP ---
    // Bob withdraws up to 90 USDD of debt, resulting in ICR of 222%
    const { collateral: B_coll, netDebt: B_netDebt } = await openTrove({
      ICR: toBN(dec(222, 16)),
      extraUSDDAmount: dec(90, 18),
      extraParams: { from: bob }
    });
    // Dennis withdraws to 150 USDD of debt, resulting in ICRs of 266%.
    const { collateral: D_coll } = await openTrove({
      ICR: toBN(dec(266, 16)),
    ...
    // Bob re-opens the trove, price 200, total debt 250 USDD, ICR = 240% (lowest one)
    ...
    th.assertIsApproximatelyEqual(
      (await collSurplusPool.getCollateral(bob, collToken.address)).toString(),
  ...
  it("liquidateTroves(): With all ICRs > 110%, Liquidates Troves until system leaves recovery mode", async () => {
    ...
    // Everyone withdraws some USDD from their Trove, resulting in different ICRs
  ...
  it("liquidateTroves(): Liquidates Troves until 1) system has left recovery mode AND 2) it reaches a Trove with ICR >= 110%", async () => {
    ...
    // price drops to 1ETH:85USDD, reducing TCR below 150%
  ...
  it("liquidateTroves(): a liquidation sequence containing Pool offsets increases the TCR", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(198, 16)),
      extraUSDDAmount: dec(101, 18),
      extraParams: { from: defaulter_1 }
    });
    await openTrove({
      ICR: toBN(dec(184, 16)),
      extraUSDDAmount: dec(217, 18),
      extraParams: { from: defaulter_2 }
    });
    await openTrove({
      ICR: toBN(dec(183, 16)),
      extraUSDDAmount: dec(328, 18),
      extraParams: { from: defaulter_3 }
    });
    await openTrove({
      ICR: toBN(dec(186, 16)),
      extraUSDDAmount: dec(431, 18),
    ...
    // Check Stability Pool has 500 USDD
  ...
  it("liquidateTroves(): does nothing if all troves have ICR > 110% and Stability Pool is empty", async () => {
    ...
    // Confirm 0 USDD in Stability Pool
    assert.equal((await stabilityPool.getTotalUSDDDeposits()).toString(), "0");
  ...
  it("liquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves", async () => {
    ...
    // Confirm USDD in Stability Pool
    assert.equal((await stabilityPool.getTotalUSDDDeposits()).toString(), spDeposit.toString());
    ...
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getCollateral(freddy, collToken.address),
      freddy_remainingCollateral
    );
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getCollateral(greta, collToken.address),
      greta_remainingCollateral
    );

    // can claim collateral
    const freddy_balanceBefore = th.toBN(await collToken.balanceOf(freddy));
    await borrowerOperations.claimCollateral({ from: freddy, gasPrice: 0 });
    const freddy_balanceAfter = th.toBN(await collToken.balanceOf(freddy));
    th.assertIsApproximatelyEqual(
      freddy_balanceAfter,
      freddy_balanceBefore.add(th.toBN(freddy_remainingCollateral))
    );

    const greta_balanceBefore = th.toBN(await collToken.balanceOf(greta));
    await borrowerOperations.claimCollateral({ from: greta, gasPrice: 0 });
    const greta_balanceAfter = th.toBN(await collToken.balanceOf(greta));
    th.assertIsApproximatelyEqual(
      greta_balanceAfter,
  ...
  it("liquidateTroves():  emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial", async () => {
    ...
    const entireColl_A = (await troveManager.Troves(alice))[1].add(
      await troveManager.getPendingCollReward(alice)
    );
    const entireDebt_A = (await troveManager.Troves(alice))[0].add(
      await troveManager.getPendingUSDDDebtReward(alice)
    );
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const { usddAmount: usddAmountD } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: dennis }
    });
    const { usddAmount: usddAmountE } = await openTrove({
      ICR: toBN(dec(133, 16)),
      extraParams: { from: erin }
    });
    const { usddAmount: usddAmountF } = await openTrove({
      ICR: toBN(dec(111, 16)),
      extraParams: { from: freddy }
    });
    ...
    assert.equal((await usddToken.balanceOf(dennis)).toString(), usddAmountD);
    assert.equal((await usddToken.balanceOf(erin)).toString(), usddAmountE);
    assert.equal((await usddToken.balanceOf(freddy)).toString(), usddAmountF);
  ...
  it("liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
    // Whale provides USDD to the SP
    const { usddAmount: W_usddAmount } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraUSDDAmount: dec(4000, 18),
      extraParams: { from: whale }
    });
    await stabilityPool.provideToSP(W_usddAmount, ZERO_ADDRESS, { from: whale });

    const { usddAmount: A_usddAmount, totalDebt: A_totalDebt, collateral: A_coll } = await openTrove(
      {
        ICR: toBN(dec(191, 16)),
    ...
    const { usddAmount: B_usddAmount, totalDebt: B_totalDebt, collateral: B_coll } = await openTrove(
      {
        ICR: toBN(dec(200, 16)),
        extraUSDDAmount: dec(240, 18),
        extraParams: { from: bob }
    ...
    await stabilityPool.provideToSP(A_usddAmount, ZERO_ADDRESS, { from: alice });
    await stabilityPool.provideToSP(B_usddAmount, ZERO_ADDRESS, { from: bob });

    const totalDeposit = W_usddAmount.add(A_usddAmount).add(B_usddAmount);
    ...
    // Check USDD in Pool
    assert.equal((await stabilityPool.getTotalUSDDDeposits()).toString(), totalDeposit);
    ...
    Total USDD in Pool: 680 USDD

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 100 + 300 + 100 = 500 USDD
    Total liquidated ETH = 1 + 3 + 1 = 5 ETH

    Whale USDD Loss: 500 * (400/680) = 294.12 USDD
    Alice USDD Loss:  500 *(40/680) = 29.41 USDD
    Bob USDD Loss: 500 * (240/680) = 176.47 USDD

    Whale remaining deposit: (400 - 294.12) = 105.88 USDD
    Alice remaining deposit: (40 - 29.41) = 10.59 USDD
    Bob remaining deposit: (240 - 176.47) = 63.53 USDD

    Whale ETH Gain: 5*0.995 * (400/680) = 2.93 ETH
    Alice ETH Gain: 5*0.995 *(40/680) = 0.293 ETH
    Bob ETH Gain: 5*0.995 * (240/680) = 1.76 ETH

    Total remaining deposits: 180 USDD
    Total ETH gain: 5*0.995 ETH */

    const USDDinSP = (await stabilityPool.getTotalUSDDDeposits()).toString();
    const ETHinSP = (await stabilityPool.getColl()).toString();

    // Check remaining USDD Deposits and ETH gain, for whale and depositors whose troves were liquidated
    const whale_Deposit_After = (await stabilityPool.getCompoundedUSDDDeposit(whale)).toString();
    ...
    const whale_ETHGain = (await stabilityPool.getDepositorCollGain(whale)).toString();
    const alice_ETHGain = (await stabilityPool.getDepositorCollGain(alice)).toString();
    const bob_ETHGain = (await stabilityPool.getDepositorCollGain(bob)).toString();
    ...
    assert.isAtMost(
      th.getDifference(
        whale_Deposit_After,
        W_usddAmount.sub(liquidatedDebt.mul(W_usddAmount).div(totalDeposit))
      ),
      100000
    );
    assert.isAtMost(
      th.getDifference(
        alice_Deposit_After,
        A_usddAmount.sub(liquidatedDebt.mul(A_usddAmount).div(totalDeposit))
      ),
      100000
    );
    assert.isAtMost(
      th.getDifference(
        bob_Deposit_After,
        B_usddAmount.sub(liquidatedDebt.mul(B_usddAmount).div(totalDeposit))
      ),
      100000
    );
    ...
    assert.isAtMost(
      th.getDifference(
        whale_ETHGain,
        th.applyLiquidationFee(liquidatedColl).mul(W_usddAmount).div(totalDeposit)
      ),
      2000
    );
    assert.isAtMost(
      th.getDifference(
        alice_ETHGain,
        th.applyLiquidationFee(liquidatedColl).mul(A_usddAmount).div(totalDeposit)
      ),
      2000
    );
    assert.isAtMost(
      th.getDifference(
        bob_ETHGain,
        th.applyLiquidationFee(liquidatedColl).mul(B_usddAmount).div(totalDeposit)
      ),
      2000
    );
    ...
    assert.isAtMost(th.getDifference(total_USDDinSP, totalDeposit.sub(liquidatedDebt)), 1000);
  ...
  it("liquidateTroves(): Liquidating troves at ICR <=100% with SP deposits does not alter their deposit or ETH gain", async () => {
    ...
    // Check USDD and ETH in Pool  before
    ...
    assert.equal(USDDinSP_Before, dec(800, 18));
    ...
    // Check USDD and ETH in Pool after
    ...
    const whale_ETHGain_After = (await stabilityPool.getDepositorCollGain(whale)).toString();
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains active", async () => {
    ...
    That leaves 50 USDD in the Pool to absorb exactly half of Carol's debt (100) */
    await troveManager.liquidateTroves(10);
  ...
  it("liquidateTroves() with a non fullfilled liquidation: still can liquidate further troves after the non-liquidated, emptied pool", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
     ...
     That leaves 97 USDD in the Pool that won’t be enough to absorb Carol,
  ...
  it("liquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 USDD in the Pool that won’t be enough to absorb any other trove */
    const tx = await troveManager.liquidateTroves(10);

    // Expect system debt reduced by 203 USDD and system coll 2.3 ETH
    const entireSystemCollAfter = await troveManager.getEntireSystemColl(collToken.address);
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt();
  ...
  it("liquidateTroves() with a non fullfilled liquidation: emits correct liquidation event values", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 USDD in the Pool which won’t be enough for any other liquidation */
    const liquidationTx = await troveManager.liquidateTroves(10);
    ...
    assert.equal(usddGasComp.toString(), dec(400, 18));

    // check collateral surplus
    const alice_remainingCollateral = A_coll.sub(A_totalDebt.mul(th.toBN(dec(11, 17))).div(price));
    const bob_remainingCollateral = B_coll.sub(B_totalDebt.mul(th.toBN(dec(11, 17))).div(price));
    th.assertIsApproximatelyEqual(
      await collSurplusPool.getCollateral(alice, collToken.address),
    ...
    // can claim collateral
    const alice_balanceBefore = th.toBN(await collToken.balanceOf(alice));
    await borrowerOperations.claimCollateral({ from: alice, gasPrice: 0 });
    const alice_balanceAfter = th.toBN(await collToken.balanceOf(alice));
    th.assertIsApproximatelyEqual(
      alice_balanceAfter,
  ...
  it("batchLiquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    // Expect system debt reduced by 203 USDD and system coll by 2 ETH
    const entireSystemCollAfter = await troveManager.getEntireSystemColl(collToken.address);
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt();
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool USDD > debt to liquidate: can liquidate troves out of order", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(200, 16)),
      extraParams: { from: alice }
    });
    ...
    // Whale provides 1000 USDD to the SP
  ...
  it("batchLiquidateTroves(): skips liquidation of troves with ICR > TCR, regardless of Stability Pool size", async () => {
    ...
    // Whale withdraws entire deposit, and re-deposits 132 USDD
    ...
    // Expected Stability Pool to fully absorb B (92 USDD + 10 virtual debt),
    // but not E as there are not enough funds in Stability Pool

    const stabilityBefore = await stabilityPool.getTotalUSDDDeposits();
    ...
    const stabilityAfter = await stabilityPool.getTotalUSDDDeposits();
  ...
  it("batchLiquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial", async () => {
    ...
    // Whale opens trove and adds 220 USDD to SP
    const spDeposit = F_totalDebt.add(G_totalDebt).add(A_totalDebt.div(toBN(2)));
    await openTrove({
      ICR: toBN(dec(285, 16)),
      extraUSDDAmount: spDeposit,
