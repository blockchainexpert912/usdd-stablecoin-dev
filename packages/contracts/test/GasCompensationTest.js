...
contract("Gas compensation tests", async accounts => {
  ...
  it("_getCompositeDebt(): returns (debt + 50) when collateral < $10 in value", async () => {
    ...
    debt = 10 USDD
    0.5% of coll = 0.04995 ETH. USD value: $9.99
    -> Expect composite debt = 10 + 200  = 2100 USDD*/
    ...
    /* ETH:USD price = 200
     ...
     debt = 0 USDD
     0.5% of coll = 0.000275 ETH. USD value: $0.055
     -> Expect composite debt = 0 + 200 = 200 USDD*/
    ...
    // debt = 200 USDD
    // 0.5% of coll = 0.004995 ETH. USD value: $6.09
    // -> Expect  composite debt =  200 + 200 = 400  USDD */
  ...
  it("getCompositeDebt(): returns (debt + 50) collateral = $10 in value", async () => {
    ...
    debt = 123.45 USDD
    0.5% of coll = 0.5 ETH. USD value: $10
    -> Expect composite debt = (123.45 + 200) = 323.45 USDD  */
  ...
  it("getCompositeDebt(): returns (debt + 50) when 0.5% of collateral > $10 in value", async () => {
    ...
    coll = 100 ETH  
    debt = 2000 USDD
    -> Expect composite debt = (2000 + 200) = 2200 USDD  */
    ...
    coll = 37.5 ETH  
    debt = 500 USDD
    -> Expect composite debt = (500 + 200) = 700 USDD  */
    ...
    coll = 94758.230582309850 ETH  
    debt = 1 billion USDD
    -> Expect composite debt = (1000000000 + 200) = 1000000200 USDD  */
    await priceFeed.setPrice("45323545420000000000000");
    const price_2 = await priceFeed.getPrice();
    ...
    ETH:USD price = 1000000 $/E (1 million)
    coll = 300000000 ETH   (300 million)
    debt = 54321.123456789 USDD
   -> Expect composite debt = (54321.123456789 + 200) = 54521.123456789 USDD */
    await priceFeed.setPrice(dec(1, 24));
    const price_3 = await priceFeed.getPrice();
  ...
  it("getCurrentICR(): Incorporates virtual debt, and returns the correct ICR for new troves", async () => {
    ...
    // E opens with 4405.45 ETH, 32598.35 USDD
  ...
  it("Gas compensation from pool-offset liquidations. All collateral paid as compensation", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(2, 18)),
      extraUSDDAmount: A_totalDebt,
      extraParams: { from: dennis }
    });
    await openTrove({
      ICR: toBN(dec(2, 18)),
      extraUSDDAmount: B_totalDebt.add(C_totalDebt),
      extraParams: { from: erin }
    });

    // D, E each provide USDD to SP
    ...
    const USDDinSP_0 = await stabilityPool.getTotalUSDDDeposits();
    ...
    // Check SP USDD has decreased due to the liquidation
    const USDDinSP_A = await stabilityPool.getTotalUSDDDeposits();
    assert.isTrue(USDDinSP_A.lte(USDDinSP_0));

    // Check ETH in SP has received the liquidation
    const ETHinSP_A = await stabilityPool.getColl();
    ...
    // Check SP USDD has decreased due to the liquidation of B
    const USDDinSP_B = await stabilityPool.getTotalUSDDDeposits();
    assert.isTrue(USDDinSP_B.lt(USDDinSP_A));

    // Check ETH in SP has received the liquidation
    const ETHinSP_B = await stabilityPool.getColl();
    assert.equal(
      ETHinSP_B.toString(),
    ...
    assert.isFalse(await th.checkRecoveryMode(contracts));
    // Liquidate B (use 0 gas price to easily check the amount the compensation amount the liquidator receives)
    const liquidatorBalance_before_C = web3.utils.toBN(await collToken.balanceOf(liquidator));
    await troveManager.liquidate(carol, { from: liquidator, gasPrice: 0 });
    const liquidatorBalance_after_C = web3.utils.toBN(await collToken.balanceOf(liquidator));
    ...
    // Check SP USDD has decreased due to the liquidation of C
    const USDDinSP_C = await stabilityPool.getTotalUSDDDeposits();
    assert.isTrue(USDDinSP_C.lt(USDDinSP_B));

    // Check ETH in SP has not changed due to the lquidation of C
    const ETHinSP_C = await stabilityPool.getColl();
    assert.equal(
      ETHinSP_C.toString(),
      aliceColl
  ...
  it("gas compensation from pool-offset liquidations: 0.5% collateral < $10 in value. Compensates $10 worth of collateral, liquidates the remainder", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(60, 18)),
      extraUSDDAmount: dec(600, 18),
      extraParams: { from: carol }
    });
    await openTrove({
      ICR: toBN(dec(80, 18)),
      extraUSDDAmount: dec(1, 23),
      extraParams: { from: dennis }
    });
    await openTrove({
      ICR: toBN(dec(80, 18)),
    ...
    // D, E each provide 10000 USDD to SP
    ...
    const USDDinSP_0 = await stabilityPool.getTotalUSDDDeposits();
    const ETHinSP_0 = await stabilityPool.getColl();
    ...
    // Check SP USDD has decreased due to the liquidation of A
    const USDDinSP_A = await stabilityPool.getTotalUSDDDeposits();
    assert.isTrue(USDDinSP_A.lt(USDDinSP_0));
  ...
  it("liquidateTroves(): full offset.  Compensates the correct amount, and liquidates the remainder", async () => {
    ...
    */
    ...
    // Check USDD in SP has decreased
    const USDDinSP_1 = await stabilityPool.getTotalUSDDDeposits();
    assert.isTrue(USDDinSP_1.lt(USDDinSP_0));
  ...
  it("liquidateTroves(): full redistribution. Compensates the correct amount, and liquidates the remainder", async () => {
    ...
    const USDDinDefaultPool_0 = await defaultPool.getUSDDDebt();
    ...
    */
    ...
    // Check USDD in DefaultPool has decreased
    const USDDinDefaultPool_1 = await defaultPool.getUSDDDebt();
    assert.isTrue(USDDinDefaultPool_1.gt(USDDinDefaultPool_0));
    ...
    const ETHinDefaultPool = (await defaultPool.getColl(collToken.address)).toString();
  ...
  it("liquidateTroves(): full offset. Liquidation event emits the correct gas compensation and total liquidated coll and debt", async () => {
    ...
    */
    ...
    // Expect liquidatedDebt = 51 + 190 + 1025 + 13510 = 14646 USDD
  ...
  it("Trove ordering: same collateral, decreasing debt. Price successively increases. Troves should maintain ordering by ICR", async () => {
    ...
    let debt = 50;
    // create 10 troves, constant coll, descending debt 100 to 90 USDD
    for (const account of _10_accounts) {
      const debtString = debt.toString().concat("000000000000000000");
      await openTrove({
        extraUSDDAmount: debtString,
        extraParams: { from: account, value: web3.utils.toBN(dec(30, "ether")) }
      });
  ...
  it("Trove ordering: increasing collateral, constant debt. Price successively increases. Troves should maintain ordering by ICR", async () => {
    ...
    let coll = 50;
    // create 20 troves, increasing collateral, constant debt = 100USDD
    for (const account of _20_accounts) {
      const collString = web3.utils.toBN(coll.toString().concat("000000000000000000"));
      await openTrove({
        extraUSDDAmount: web3.utils.toBN(dec(100, 18)),
