...
contract("TroveManager", async accounts => {
  ...
  const getOpenTroveTotalDebt = async usddAmount => th.getOpenTroveTotalDebt(contracts, usddAmount);
  ...
  const withdrawUSDD = async params => th.withdrawUSDD(contracts, params);
  ...
  it("liquidate(): closes a Trove that has ICR < MCR", async () => {
    ...
    // Alice increases debt to 180 USDD, lowering her ICR to 1.11
    const A_USDDWithdrawal = await getNetBorrowingAmount(dec(130, 18));

    const targetICR = toBN("1111111111111111111");
    await withdrawUSDD({ ICR: targetICR, extraParams: { from: alice } });
    ...
    // price drops to 1ETH:100USDD, reducing Alice's ICR below MCR
  ...
  it("liquidate(): decreases ActivePool ETH and USDDDebt by correct amounts", async () => {
    // --- SETUP ---
    ...
    // check ActivePool ETH and USDD debt before
    const activePool_ETH_Before = (await activePool.getColl(collToken.address)).toString();
    const activePool_COLL_Before = (await collToken.balanceOf(activePool.address)).toString();
    const activePool_USDDDebt_Before = (await activePool.getUSDDDebt()).toString();
    ...
    th.assertIsApproximatelyEqual(activePool_USDDDebt_Before, A_totalDebt.add(B_totalDebt));

    // price drops to 1ETH:100USDD, reducing Bob's ICR below MCR
    await priceFeed.setPrice("100000000000000000000");

    // Confirm system is not in Recovery Mode
    assert.isFalse(await th.checkRecoveryMode(contracts));

    /* close Bob's Trove. Should liquidate his ether and USDD, 
    leaving Alice’s ether and USDD debt in the ActivePool. */
    await troveManager.liquidate(bob, { from: owner });

    // check ActivePool ETH and USDD debt
    const activePool_ETH_After = (await activePool.getColl(collToken.address)).toString();
    const activePool_COLL_After = (await collToken.balanceOf(activePool.address)).toString();
    const activePool_USDDDebt_After = (await activePool.getUSDDDebt()).toString();

    assert.equal(activePool_ETH_After, A_collateral);
    assert.equal(activePool_COLL_After, A_collateral);
    th.assertIsApproximatelyEqual(activePool_USDDDebt_After, A_totalDebt);
  });

  it("liquidate(): increases DefaultPool ETH and USDD debt by correct amounts", async () => {
    // --- SETUP ---
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(4, 18)),
      extraParams: { from: alice }
    ...
    // check DefaultPool ETH and USDD debt before
    const defaultPool_ETH_Before = await defaultPool.getColl(collToken.address);
    const defaultPool_RawEther_Before = (await collToken.balanceOf(defaultPool.address)).toString();
    const defaultPool_USDDDebt_Before = (await defaultPool.getUSDDDebt()).toString();

    assert.equal(defaultPool_ETH_Before, "0");
    assert.equal(defaultPool_RawEther_Before, "0");
    assert.equal(defaultPool_USDDDebt_Before, "0");
    ...
    // check after
    const defaultPool_ETH_After = (await defaultPool.getColl(collToken.address)).toString();
    const defaultPool_COLL_After = (await collToken.balanceOf(defaultPool.address)).toString();
    const defaultPool_USDDDebt_After = (await defaultPool.getUSDDDebt()).toString();

    const defaultPool_ETH = th.applyLiquidationFee(B_collateral);
    assert.equal(defaultPool_ETH_After, defaultPool_ETH);
    assert.equal(defaultPool_COLL_After, defaultPool_ETH);
    th.assertIsApproximatelyEqual(defaultPool_USDDDebt_After, B_totalDebt);
  });
  ...
  it("liquidate(): updates the snapshots of total stakes and total collateral", async () => {
    ...
    // close Bob's Trove.  His ether*0.995 and USDD should be added to the DefaultPool.
    await troveManager.liquidate(bob, { from: owner });
  ...
  it("liquidate(): updates the L_ETH and L_USDDDebt reward-per-unit-staked totals", async () => {
    // --- SETUP ---
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(8, 18)),
      extraParams: { from: alice }
    ...
    // price drops to 1ETH:100USDD, reducing Carols's ICR below MCR
    await priceFeed.setPrice("100000000000000000000");
    ...
    // Carol's ether*0.995 and USDD should be added to the DefaultPool.
    const L_ETH_AfterCarolLiquidated = await troveManager.L_COLL();
    const L_USDDDebt_AfterCarolLiquidated = await troveManager.L_USDDDebt();

    const L_ETH_expected_1 = th
      .applyLiquidationFee(C_collateral)
      .mul(mv._1e18BN)
      .div(A_collateral.add(B_collateral));
    const L_USDDDebt_expected_1 = C_totalDebt.mul(mv._1e18BN).div(A_collateral.add(B_collateral));
    assert.isAtMost(th.getDifference(L_ETH_AfterCarolLiquidated, L_ETH_expected_1), 100);
    assert.isAtMost(th.getDifference(L_USDDDebt_AfterCarolLiquidated, L_USDDDebt_expected_1), 100);

    // Bob now withdraws USDD, bringing his ICR to 1.11
    const { increasedTotalDebt: B_increasedTotalDebt } = await withdrawUSDD({
      ICR: toBN(dec(111, 16)),
      extraParams: { from: bob }
    });
    ...
    // price drops to 1ETH:50USDD, reducing Bob's ICR below MCR
   ...
   L_USDDDebt = (180 / 20) + (890 / 10) = 98 USDD */
    const L_ETH_AfterBobLiquidated = await troveManager.L_COLL();
    const L_USDDDebt_AfterBobLiquidated = await troveManager.L_USDDDebt();
    ...
    const L_USDDDebt_expected_2 = L_USDDDebt_expected_1.add(
      B_totalDebt.add(B_increasedTotalDebt)
        .add(B_collateral.mul(L_USDDDebt_expected_1).div(mv._1e18BN))
        .mul(mv._1e18BN)
        .div(A_collateral)
    );
    assert.isAtMost(th.getDifference(L_ETH_AfterBobLiquidated, L_ETH_expected_2), 100);
    assert.isAtMost(th.getDifference(L_USDDDebt_AfterBobLiquidated, L_USDDDebt_expected_2), 100);
  });
  ...
  it("liquidate(): Liquidates undercollateralized trove if there are two troves in the system", async () => {
    ...
    // Alice creates a single trove with 0.7 ETH and a debt of 70 USDD, and provides 10 USDD to SP
    const { collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraParams: { from: alice }
    });

    // Alice proves 10 USDD to SP
  ...
  it("liquidate(): Given the same price and no other trove changes, complete Pool offsets restore the TCR to its value prior to the defaulters opening troves", async () => {
    // Whale provides USDD to SP
    const spDeposit = toBN(dec(100, 24));
    await openTrove({
      ICR: toBN(dec(4, 18)),
      extraUSDDAmount: spDeposit,
  ...
  it("liquidate(): a pure redistribution reduces the TCR only as a result of compensation", async () => {
    ...
    const entireSystemCollBefore = await troveManager.getEntireSystemColl(collToken.address);
  ...
  it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
    ...
    await usddToken.transfer(dennis, spDeposit, { from: bob });

    //Dennis provides USDD to SP
    ...
    const dennis_Deposit_Before = (await stabilityPool.getCompoundedUSDDDeposit(dennis)).toString();
    const dennis_ETHGain_Before = (await stabilityPool.getDepositorCollGain(dennis)).toString();
    ...
    const dennis_Deposit_After = (await stabilityPool.getCompoundedUSDDDeposit(dennis)).toString();
    const dennis_ETHGain_After = (await stabilityPool.getDepositorCollGain(dennis)).toString();
  ...
  it("liquidate(): liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and ETH gain", async () => {
    ...
    /* Alice's USDD Loss = (300 / 400) * 200 = 150 USDD
       Alice's ETH gain = (300 / 400) * 2*0.995 = 1.4925 ETH

       Bob's USDDLoss = (100 / 400) * 200 = 50 USDD
       Bob's ETH gain = (100 / 400) * 2*0.995 = 0.4975 ETH

     Check Bob' SP deposit has been reduced to 50 USDD, and his ETH gain has increased to 1.5 ETH. */
    const alice_Deposit_After = (await stabilityPool.getCompoundedUSDDDeposit(alice)).toString();
    ...
    const bob_Deposit_After = await stabilityPool.getCompoundedUSDDDeposit(bob);
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } });
    const { usddAmount: A_usddAmount } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraUSDDAmount: toBN(dec(300, 18)),
      extraParams: { from: alice }
    });
    const { usddAmount: B_usddAmount } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraUSDDAmount: toBN(dec(200, 18)),
      extraParams: { from: bob }
    });
    const { usddAmount: C_usddAmount } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraUSDDAmount: toBN(dec(100, 18)),
      extraParams: { from: carol }
    });
    ...
    // Liquidate A, B and C
    const activeUSDDDebt_0 = await activePool.getUSDDDebt();
    const defaultUSDDDebt_0 = await defaultPool.getUSDDDebt();

    await troveManager.liquidate(alice);
    const activeUSDDDebt_A = await activePool.getUSDDDebt();
    const defaultUSDDDebt_A = await defaultPool.getUSDDDebt();

    await troveManager.liquidate(bob);
    const activeUSDDDebt_B = await activePool.getUSDDDebt();
    const defaultUSDDDebt_B = await defaultPool.getUSDDDebt();
    ...
    assert.equal((await usddToken.balanceOf(alice)).toString(), A_usddAmount);
    assert.equal((await usddToken.balanceOf(bob)).toString(), B_usddAmount);
    assert.equal((await usddToken.balanceOf(carol)).toString(), C_usddAmount);
  });
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    // Defaulter opens with 60 USDD, 0.6 ETH
    ...
    /* Liquidate defaulter. 30 USDD and 0.3 ETH is distributed between A, B and C.

    A receives (30 * 2/4) = 15 USDD, and (0.3*2/4) = 0.15 ETH
    B receives (30 * 1/4) = 7.5 USDD, and (0.3*1/4) = 0.075 ETH
    C receives (30 * 1/4) = 7.5 USDD, and (0.3*1/4) = 0.075 ETH
    */
    await troveManager.liquidate(defaulter_1);
  ...
  it("liquidate(): when SP > 0, triggers DEFT reward event - increases the sum G", async () => {
    ...
    assert.equal(await stabilityPool.getTotalUSDDDeposits(), dec(100, 18));

    const G_Before = await stabilityPool.epochToScaleToG(0, 0);

    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider);

    // Price drops to 1ETH:100USDD, reducing defaulters to below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice();
    assert.isFalse(await th.checkRecoveryMode(contracts));
  ...
  it("liquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards", async () => {
    ...
    // A adds 10 USDD to the SP, but less than C's debt
    ...
    await borrowerOperations.repayUSDD(dec(1, 18), D, D, { from: D });
    await borrowerOperations.repayUSDD(dec(1, 18), E, E, { from: E });
    ...
    const pendingETH_C = await troveManager.getPendingCollReward(C);
    const pendingUSDDDebt_C = await troveManager.getPendingUSDDDebtReward(C);
    const defaultPoolETH = await defaultPool.getColl(collToken.address);
    const defaultPoolUSDDDebt = await defaultPool.getUSDDDebt();
    assert.isTrue(pendingETH_C.lte(defaultPoolETH));
    assert.isTrue(pendingUSDDDebt_C.lte(defaultPoolUSDDDebt));
    //Check only difference is dust
    assert.isAtMost(th.getDifference(pendingETH_C, defaultPoolETH), 1000);
    assert.isAtMost(th.getDifference(pendingUSDDDebt_C, defaultPoolUSDDDebt), 1000);
  ...
  it("liquidateTroves(): closes every Trove with ICR < MCR, when n > number of undercollateralized troves", async () => {
    ...
    // Price drops to 1ETH:100USDD, reducing Bob and Carol's ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice();
  ...
  it("liquidateTroves(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    // Liquidate defaulter. 30 USDD and 0.3 ETH is distributed uniformly between A, B and C. Each receive 10 USDD, 0.1 ETH
    await troveManager.liquidate(defaulter_1);
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const D_balanceBefore = await usddToken.balanceOf(dennis);
    const E_balanceBefore = await usddToken.balanceOf(erin);
    const F_balanceBefore = await usddToken.balanceOf(flyn);
    ...
    assert.equal((await usddToken.balanceOf(dennis)).toString(), D_balanceBefore);
    assert.equal((await usddToken.balanceOf(erin)).toString(), E_balanceBefore);
    assert.equal((await usddToken.balanceOf(flyn)).toString(), F_balanceBefore);
  });

  it("liquidateTroves(): A liquidation sequence containing Pool offsets increases the TCR", async () => {
    // Whale provides 500 USDD to SP
    await openTrove({
      ICR: toBN(dec(100, 18)),
      extraUSDDAmount: toBN(dec(500, 18)),
      extraParams: { from: whale }
    });
    ...
    // Check pool has 500 USDD
    assert.equal((await stabilityPool.getTotalUSDDDeposits()).toString(), dec(500, 18));
    ...
    assert.equal((await stabilityPool.getTotalUSDDDeposits()).toString(), "0");
  ...
  it("liquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
    // Whale provides 400 USDD to the SP
    ...
    const { collateral: A_coll, totalDebt: A_debt } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraUSDDAmount: A_deposit,
      extraParams: { from: alice }
    });
    const { collateral: B_coll, totalDebt: B_debt } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraUSDDAmount: B_deposit,
      extraParams: { from: bob }
    });
    const { collateral: C_coll, totalDebt: C_debt } = await openTrove({
      ICR: toBN(dec(2, 18)),
    ...
    // Check 800 USDD in Pool
    const totalDeposits = whaleDeposit.add(A_deposit).add(B_deposit);
    assert.equal((await stabilityPool.getTotalUSDDDeposits()).toString(), totalDeposits);
    ...
    Total USDD in Pool: 800 USDD

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 150 + 350 + 150 = 650 USDD
    Total liquidated ETH = 1.1 + 3.1 + 1.1 = 5.3 ETH

    whale usdd loss: 650 * (400/800) = 325 usdd
    alice usdd loss:  650 *(100/800) = 81.25 usdd
    bob usdd loss: 650 * (300/800) = 243.75 usdd

    whale remaining deposit: (400 - 325) = 75 usdd
    alice remaining deposit: (100 - 81.25) = 18.75 usdd
    bob remaining deposit: (300 - 243.75) = 56.25 usdd
    ...
    Total remaining deposits: 150 USDD
    Total ETH gain: 4.975 ETH */

    // Check remaining USDD Deposits and ETH gain, for whale and depositors whose troves were liquidated
    const whale_Deposit_After = await stabilityPool.getCompoundedUSDDDeposit(whale);
    const alice_Deposit_After = await stabilityPool.getCompoundedUSDDDeposit(alice);
    const bob_Deposit_After = await stabilityPool.getCompoundedUSDDDeposit(bob);
    ...
    const total_USDDinSP = (await stabilityPool.getTotalUSDDDeposits()).toString();
    const total_ETHinSP = (await stabilityPool.getColl()).toString();

    assert.isAtMost(th.getDifference(total_USDDinSP, totalDeposits.sub(liquidatedDebt)), 1000);
    assert.isAtMost(th.getDifference(total_ETHinSP, th.applyLiquidationFee(liquidatedColl)), 1000);
  });

  it("liquidateTroves(): when SP > 0, triggers DEFT reward event - increases the sum G", async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } });
  ...
  it("batchLiquidateTroves(): closes every trove with ICR < MCR in the given array", async () => {
    ...
    // Price drops to 1ETH:100USDD, reducing A, B, C ICR below MCR
    await priceFeed.setPrice(dec(100, 18));
    const price = await priceFeed.getPrice();
  ...
  it("batchLiquidateTroves(): skips if trove is non-existent", async () => {
    ...
    th.assertIsApproximatelyEqual(
      (await stabilityPool.getTotalUSDDDeposits()).toString(),
  ...
  it("batchLiquidateTroves(): skips if a trove has been closed", async () => {
    ...
    await usddToken.transfer(carol, dec(100, 18), { from: whale });
  ...
  it("batchLiquidateTroves: when SP > 0, triggers DEFT reward event - increases the sum G", async () => {
    await openTrove({ ICR: toBN(dec(100, 18)), extraParams: { from: whale } });
  ...
  it("getRedemptionHints(): gets the address of the first Trove and the final ICR of the last Trove involved in a redemption", async () => {
    ...
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(310, 16)),
      extraUSDDAmount: partialRedemptionAmount,
      extraParams: { from: alice }
    });
  ...
  it("getRedemptionHints(): returns 0 as partialRedemptionHintNICR when reaching _maxIterations", async () => {
    ...
    // Get hints for a redemption of 170 + 30 + some extra USDD. At least 3 iterations are needed
  ...
  it("redeemCollateral(): cancels the provided USDD with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether", async () => {
    // --- SETUP ---
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(310, 16)),
      extraUSDDAmount: dec(10, 18),
      extraParams: { from: alice }
    });
    const { netDebt: B_netDebt } = await openTrove({
      ICR: toBN(dec(290, 16)),
      extraUSDDAmount: dec(8, 18),
      extraParams: { from: bob }
    });
    ...
    await openTrove({
      ICR: toBN(dec(100, 18)),
      extraUSDDAmount: redemptionAmount,
      extraParams: { from: dennis }
    });

    const dennis_ETHBalance_Before = toBN(await collToken.balanceOf(dennis));

    const dennis_USDDBalance_Before = await usddToken.balanceOf(dennis);
    ...
    // Find hints for redeeming 20 USDD
    const { firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
      redemptionAmount,
      price,
      0
    ...
    // Dennis redeems 20 USDD
    // Don't pay for gas, as it makes it easier to calculate the received Ether
    const redemptionTx = await troveManager.methods[
    ...
    /* check that Dennis' redeemed 20 USDD has been cancelled with debt from Bobs's Trove (8) and Carol's Trove (10).
    The remaining lot (2) is sent to Alice's Trove, who had the best ICR.
    It leaves her with (3) USDD debt + 50 for gas compensation. */
    th.assertIsApproximatelyEqual(alice_debt_After, A_totalDebt.sub(partialRedemptionAmount));
    assert.equal(bob_debt_After, "0");
    assert.equal(carol_debt_After, "0");

    const dennis_ETHBalance_After = toBN(await collToken.balanceOf(dennis));
    const receivedETH = dennis_ETHBalance_After.sub(dennis_ETHBalance_Before);

    const expectedTotalETHDrawn = redemptionAmount.div(toBN(200)); // convert redemptionAmount USDD to ETH, at ETH:USD price 200
    ...
    const dennis_USDDBalance_After = (await usddToken.balanceOf(dennis)).toString();
    assert.equal(dennis_USDDBalance_After, dennis_USDDBalance_Before.sub(redemptionAmount));
  });
  ...
  it("redeemCollateral(): ends the redemption sequence when the token redemption request has been filled", async () => {
    ...
    // open trove from redeemer.  Redeemer has highest ICR (100ETH, 100 USDD), 20000%
    const { usddAmount: F_usddAmount } = await openTrove({
      ICR: toBN(dec(200, 18)),
      extraUSDDAmount: redemptionAmount.mul(toBN(2)),
    ...
    await troveManager.methods[
      "redeemCollateral(uint256,address,address,address,uint256,uint256,uint256)"
    ](redemptionAmount, alice, alice, alice, 0, 0, th._100pct, {
      from: flyn
    });

    // Check Flyn's redemption has reduced his balance from 100 to (100-60) = 40 USDD
    const flynBalance = await usddToken.balanceOf(flyn);
    th.assertIsApproximatelyEqual(flynBalance, F_usddAmount.sub(redemptionAmount));
  ...
  it("redeemCollateral(): ends the redemption sequence when max iterations have been reached", async () => {
    ...
    await troveManager.methods[
      "redeemCollateral(uint256,address,address,address,uint256,uint256,uint256)"
    ](attemptedRedemptionAmount, alice, alice, alice, 0, 2, th._100pct, { from: flyn });

    // Check Flyn's redemption has reduced his balance from 100 to (100-40) = 60 USDD
    const flynBalance = (await usddToken.balanceOf(flyn)).toString();
    th.assertIsApproximatelyEqual(flynBalance, F_usddAmount.sub(redemptionAmount));
  ...
  it("redeemCollateral(): performs partial redemption if resultant debt is > minimum net debt", async () => {
    ...
    await openTrove({
      ...
      upperHint: B,
      lowerHint: B,
      extraParams: { from: B, value: toBN(dec(1000, "ether")) }
    });
    ...
    await openTrove({
      ...
      upperHint: C,
      lowerHint: C,
      extraParams: { from: C, value: toBN(dec(1000, "ether")) }
    });

    // A and C send all their tokens to B
    await usddToken.transfer(B, await usddToken.balanceOf(A), { from: A });
    await usddToken.transfer(B, await usddToken.balanceOf(C), { from: C });
    ...
    // USDD redemption is 55000 US
    const USDDRedemption = dec(55000, 18);
    const tx1 = await th.redeemCollateralAndGetTxObject(B, contracts, USDDRedemption, th._100pct);
  ...
  it("redeemCollateral(): doesn't perform partial redemption if resultant debt would be < minimum net debt", async () => {
    await openTrove({
      maxFeePercentage: th._100pct,
      usddAmount: await getOpenTroveUSDDAmount(dec(6000, 18)),
      upperHint: A,
      lowerHint: A,
      extraParams: { from: A, value: toBN(dec(1000, "ether")) }
    });
  ...
  it("redeemCollateral(): doesnt perform the final partial redemption in the sequence if the hint is out-of-date", async () => {
    ...
    // Oops, another transaction gets in the way
    {
      ...
      // Alice redeems 1 USDD from Carol's Trove
      await troveManager.methods[
        "redeemCollateral(uint256,address,address,address,uint256,uint256,uint256)"
      ](
        frontRunRedepmtion,
    ...
    // Dennis tries to redeem 20 USDD
    const redemptionTx = await troveManager.methods[
      "redeemCollateral(uint256,address,address,address,uint256,uint256,uint256)"
    ](
      redemptionAmount,
    ...
    // Since Alice already redeemed 1 USDD from Carol's Trove, Dennis was  able to redeem:
    //  - 9 USDD from Carol's
    //  - 8 USDD from Bob's
    // for a total of 17 USDD.

    // Dennis calculated his hint for redeeming 2 USDD from Alice's Trove, but after Alice's transaction
    // got in the way, he would have needed to redeem 3 USDD to fully complete his redemption of 20 USDD.
    ...
    const expectedTotalETHDrawn = fullfilledRedemptionAmount.sub(frontRunRedepmtion).div(toBN(200)); // redempted USDD converted to ETH, at ETH:USD price 200
    ...
    const dennis_USDDBalance_After = (await usddToken.balanceOf(dennis)).toString();
    th.assertIsApproximatelyEqual(
      dennis_USDDBalance_After,
      dennis_USDDBalance_Before.sub(fullfilledRedemptionAmount.sub(frontRunRedepmtion))
    );
  });
  ...
  it.skip("redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool", async () => {
    ...
    const amount = await getOpenTroveUSDDAmount(dec(110, 18));
    await openTrove({ ICR: toBN(dec(20, 18)), extraParams: { from: alice } });
    await openTrove({
      ICR: toBN(dec(133, 16)),
      extraUSDDAmount: amount,
      extraParams: { from: bob }
    });

    await usddToken.transfer(carol, amount, { from: bob });
    ...
    const carol_ETHBalance_Before = toBN(await collToken.balanceOf(carol));
    ...
    const carol_ETHBalance_After = toBN(await collToken.balanceOf(carol));

    const expectedTotalETHDrawn = toBN(amount).div(toBN(100)); // convert 100 USDD to ETH at ETH:USD price of 100
    const expectedReceivedETH = expectedTotalETHDrawn.sub(ETHFee);

    const receivedETH = carol_ETHBalance_After.sub(carol_ETHBalance_Before);
    assert.isTrue(expectedReceivedETH.eq(receivedETH));

    const carol_USDDBalance_After = (await usddToken.balanceOf(carol)).toString();
    assert.equal(carol_USDDBalance_After, "0");
  });
  ...
  it("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
    ...
    const { usddAmount: B_usddAmount, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(133, 16)),
      extraUSDDAmount: A_debt,
      extraParams: { from: bob }
    });

    await usddToken.transfer(carol, B_usddAmount, { from: bob });
  ...
  it("redeemCollateral(): finds the last Trove with ICR == 110% even if there is more than one", async () => {
    ...
    const tx = await troveManager.methods[
      "redeemCollateral(uint256,address,address,address,uint256,uint256,uint256)"
    ](
      redemptionAmount,
  ...
  it("redeemCollateral(): reverts when argument _amount is 0", async () => {
    ...
    // Alice opens trove and transfers 500USDD to Erin, the would-be redeemer
    await openTrove({
      ICR: toBN(dec(200, 16)),
      extraUSDDAmount: dec(500, 18),
      extraParams: { from: alice }
    });
    await usddToken.transfer(erin, dec(500, 18), { from: alice });
    ...
    const redemptionTxPromise = troveManager.methods[
      "redeemCollateral(uint256,address,address,address,uint256,uint256,uint256)"
    ](0, erin, erin, erin, 0, 0, th._100pct, { from: erin });
  ...
  it("redeemCollateral(): reverts if max fee > 100%", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(400, 16)),
      extraUSDDAmount: dec(30, 18),
      extraParams: { from: C }
    });
    await openTrove({
      ICR: toBN(dec(400, 16)),
      extraUSDDAmount: dec(40, 18),
      extraParams: { from: D }
    });
  ...
  it("redeemCollateral(): reverts if fee exceeds max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraUSDDAmount: dec(80, 18),
      extraParams: { from: A }
    });
    const { totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraUSDDAmount: dec(90, 18),
      extraParams: { from: B }
    });
    const { totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraUSDDAmount: dec(100, 18),
      extraParams: { from: C }
    });
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt);

    // Check total USDD supply
    ...
    // USDD redemption is 27 USD: a redemption that incurs a fee of 27/(270 * 2) = 5%
    const attemptedUSDDRedemption = expectedTotalSupply.div(toBN(10));

    // Max fee is <5%
    const lessThan5pct = "49999999999999999";
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedUSDDRedemption, lessThan5pct),
    ...
    // Max fee is 1%
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedUSDDRedemption, dec(1, 16)),
      "Fee exceeded provided maximum"
    );
    ...
    // Max fee is 3.754%
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedUSDDRedemption, dec(3754, 13)),
      "Fee exceeded provided maximum"
    );
    ...
    // Max fee is 0.5%
    await assertRevert(
      th.redeemCollateralAndGetTxObject(A, contracts, attemptedUSDDRedemption, dec(5, 15)),
      "Fee exceeded provided maximum"
    );
  });
  ...
  it("redeemCollateral(): succeeds if fee is less than max fee percentage", async () => {
    const { totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraUSDDAmount: dec(9500, 18),
      extraParams: { from: A }
    });
    const { totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(395, 16)),
      extraUSDDAmount: dec(9000, 18),
      extraParams: { from: B }
    });
    const { totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(390, 16)),
      extraUSDDAmount: dec(10000, 18),
      extraParams: { from: C }
    });
    const expectedTotalSupply = A_totalDebt.add(B_totalDebt).add(C_totalDebt);
    ...
    // USDD redemption fee with 10% of the supply will be 0.5% + 1/(10*2)
    const attemptedUSDDRedemption = expectedTotalSupply.div(toBN(10));

    // Attempt with maxFee > 5.5%
    const price = await priceFeed.getPrice();
    const ETHDrawn = attemptedUSDDRedemption.mul(mv._1e18BN).div(price);
  ...
  it("redeemCollateral(): doesn't affect the Stability Pool deposits or ETH gain of redeemed-from troves", async () => {
    ...
    const { totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(195, 16)),
      extraUSDDAmount: dec(200, 18),
      extraParams: { from: carol }
    });
    const { totalDebt: D_totalDebt } = await openTrove({
      ICR: toBN(dec(190, 16)),
      extraUSDDAmount: dec(400, 18),
      extraParams: { from: dennis }
    });
    ...
    // Alice opens trove and transfers USDD to Erin, the would-be redeemer
    await openTrove({
      ICR: toBN(dec(300, 16)),
      extraUSDDAmount: redemptionAmount,
      extraParams: { from: alice }
    });
    await usddToken.transfer(erin, redemptionAmount, { from: alice });
    ...
    const bob_SPDeposit_before = (await stabilityPool.getCompoundedUSDDDeposit(bob)).toString();
    const carol_SPDeposit_before = (await stabilityPool.getCompoundedUSDDDeposit(carol)).toString();
    const dennis_SPDeposit_before = (
      await stabilityPool.getCompoundedUSDDDeposit(dennis)
    ).toString();
    ...
    // Check the remaining USDD and ETH in Stability Pool after liquidation is non-zero
    const USDDinSP = await stabilityPool.getTotalUSDDDeposits();
    const ETHinSP = await stabilityPool.getColl();
    assert.isTrue(USDDinSP.gte(mv._zeroBN));
    ...
    const bob_SPDeposit_after = (await stabilityPool.getCompoundedUSDDDeposit(bob)).toString();
    const carol_SPDeposit_after = (await stabilityPool.getCompoundedUSDDDeposit(carol)).toString();
    const dennis_SPDeposit_after = (await stabilityPool.getCompoundedUSDDDeposit(dennis)).toString();
  ...
  it("redeemCollateral(): caller can redeem their entire USDDToken balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({
      ICR: toBN(dec(20, 18)),
      extraParams: { from: whale }
    });

    // Alice opens trove and transfers 400 USDD to Erin, the would-be redeemer
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraUSDDAmount: dec(400, 18),
      extraParams: { from: alice }
    });
    await usddToken.transfer(erin, dec(400, 18), { from: alice });

    // Check Erin's balance before
    const erin_balance_before = await usddToken.balanceOf(erin);
    assert.equal(erin_balance_before, dec(400, 18));

    // B, C, D open trove
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraUSDDAmount: dec(590, 18),
      extraParams: { from: bob }
    });
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraUSDDAmount: dec(1990, 18),
      extraParams: { from: carol }
    });
    ...
    const activePool_coll_before = await activePool.getColl(collToken.address);
    ...
    // Erin attempts to redeem 400 USDD
    const { firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
      dec(400, 18),
      price,
      0
    ...
    // Check activePool debt reduced by  400 USDD
    const activePool_debt_after = await activePool.getUSDDDebt();
    ...
    const activePool_coll_after = await activePool.getColl(collToken.address);
    ...
    const erin_balance_after = (await usddToken.balanceOf(erin)).toString();
    assert.equal(erin_balance_after, "0");
  });

  it("redeemCollateral(): reverts when requested redemption amount exceeds caller's USDD token balance", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({
      ICR: toBN(dec(20, 18)),
      extraParams: { from: whale }
    });
    ...
    // Get active debt and coll before redemption
    const activePool_debt_before = await activePool.getUSDDDebt();
    const activePool_coll_before = (await activePool.getColl(collToken.address)).toString();
    ...
    // Erin tries to redeem 1000 USDD
    try {
      ...
      assert.include(
        error.message,
        "Requested redemption amount must be <= user's USDD token balance"
      );
    }

    // Erin tries to redeem 401 USDD
    try {
      ({ firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
    ...
    // Erin tries to redeem 239482309 USDD
    try {
      ({ firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
    ...
    // Erin tries to redeem 2^256 - 1 USDD
  ...
  it("redeemCollateral(): value of issued ETH == face value of redeemed USDD (assuming 1 USDD has value of $1)", async () => {
    const { collateral: W_coll } = await openTrove({
      ICR: toBN(dec(20, 18)),
      extraParams: { from: whale }
    });

    // Alice opens trove and transfers 1000 USDD each to Erin, Flyn, Graham
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
      ICR: toBN(dec(400, 16)),
      extraUSDDAmount: dec(4990, 18),
      extraParams: { from: alice }
    });
    await usddToken.transfer(erin, dec(1000, 18), { from: alice });
    await usddToken.transfer(flyn, dec(1000, 18), { from: alice });
    await usddToken.transfer(graham, dec(1000, 18), { from: alice });

    // B, C, D open trove
    const { collateral: B_coll } = await openTrove({
      ICR: toBN(dec(300, 16)),
      extraUSDDAmount: dec(1590, 18),
      extraParams: { from: bob }
    });
    const { collateral: C_coll } = await openTrove({
      ICR: toBN(dec(600, 16)),
      extraUSDDAmount: dec(1090, 18),
      extraParams: { from: carol }
    });
    ...
    const _120_USDD = "120000000000000000000";
    const _373_USDD = "373000000000000000000";
    const _950_USDD = "950000000000000000000";

    // Check Ether in activePool
    const activeETH_0 = await activePool.getColl(collToken.address);
    assert.equal(activeETH_0, totalColl.toString());

    let firstRedemptionHint;
    let partialRedemptionHintNICR;

    // Erin redeems 120 USDD
    ({ firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
      _120_USDD,
      price,
      0
    ...
    const redemption_1 = await troveManager.methods[
      "redeemCollateral(uint256,address,address,address,uint256,uint256,uint256)"
    ](
      _120_USDD,
      firstRedemptionHint,
    ...
    /* 120 USDD redeemed.  Expect $120 worth of ETH removed. At ETH:USD price of $200, 
    ETH removed = (120/200) = 0.6 ETH
    Total active ETH = 280 - 0.6 = 279.4 ETH */

    const activeETH_1 = await activePool.getColl(collToken.address);
    assert.equal(
      activeETH_1.toString(),
      activeETH_0.sub(toBN(_120_USDD).mul(mv._1e18BN).div(price))
    );

    // Flyn redeems 373 USDD
    ({ firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
      _373_USDD,
      price,
      0
    ...
    const redemption_2 = await troveManager.methods[
      "redeemCollateral(uint256,address,address,address,uint256,uint256,uint256)"
    ](
      _373_USDD,
      firstRedemptionHint,
    ...
    /* 373 USDD redeemed.  Expect $373 worth of ETH removed. At ETH:USD price of $200, 
    ETH removed = (373/200) = 1.865 ETH
    Total active ETH = 279.4 - 1.865 = 277.535 ETH */
    const activeETH_2 = await activePool.getColl(collToken.address);
    assert.equal(
      activeETH_2.toString(),
      activeETH_1.sub(toBN(_373_USDD).mul(mv._1e18BN).div(price))
    );

    // Graham redeems 950 USDD
    ({ firstRedemptionHint, partialRedemptionHintNICR } = await hintHelpers.getRedemptionHints(
      _950_USDD,
      price,
      0
    ...
    const redemption_3 = await troveManager.methods[
      "redeemCollateral(uint256,address,address,address,uint256,uint256,uint256)"
    ](
      _950_USDD,
      firstRedemptionHint,
    ...
    /* 950 USDD redeemed.  Expect $950 worth of ETH removed. At ETH:USD price of $200, 
    ETH removed = (950/200) = 4.75 ETH
    Total active ETH = 277.535 - 4.75 = 272.785 ETH */
    const activeETH_3 = (await activePool.getColl(collToken.address)).toString();
    assert.equal(
      activeETH_3.toString(),
      activeETH_2.sub(toBN(_950_USDD).mul(mv._1e18BN).div(price))
    );
  });
  ...
  it("redeemCollateral(): reverts if there is zero outstanding system debt", async () => {
    // --- SETUP --- illegally mint USDD to Bob
    await usddToken.unprotectedMint(bob, dec(100, 18));

    assert.equal(await usddToken.balanceOf(bob), dec(100, 18));
    ...
    // Bob tries to redeem his illegally obtained USDD
    try {
      const redemptionTx = await troveManager.methods[
        "redeemCollateral(uint256,address,address,address,uint256,uint256,uint256)"
      ](
  ...
  it("redeemCollateral(): reverts if caller's tries to redeem more than the outstanding system debt", async () => {
    // --- SETUP --- illegally mint USDD to Bob
    await usddToken.unprotectedMint(bob, "101000000000000000000");

    assert.equal(await usddToken.balanceOf(bob), "101000000000000000000");
    ...
    th.assertIsApproximatelyEqual((await activePool.getUSDDDebt()).toString(), totalDebt);
    ...
    // Bob attempts to redeem his ill-gotten 101 USDD, from a system that has 100 USDD outstanding debt
    try {
      const redemptionTx = await troveManager.methods[
        "redeemCollateral(uint256,address,address,address,uint256,uint256,uint256)"
      ](
  ...
  it("redeemCollateral(): a redemption made when base rate is zero increases the base rate", async () => {
    ...
    const A_balanceBefore = await usddToken.balanceOf(A);

    await th.redeemCollateral(A, contracts, dec(10, 18));

    // Check A's balance has decreased by 10 USDD
    assert.equal(await usddToken.balanceOf(A), A_balanceBefore.sub(toBN(dec(10, 18))).toString());
  ...
  it("redeemCollateral(): a redemption made when base rate is non-zero increases the base rate, for negligible time passed", async () => {
    ...
    const A_balanceBefore = await usddToken.balanceOf(A);
    const B_balanceBefore = await usddToken.balanceOf(B);
    ...
    // Check B's balance has decreased by 10 USDD
    assert.equal(await usddToken.balanceOf(B), B_balanceBefore.sub(toBN(dec(10, 18))).toString());
  ...
  it("redeemCollateral(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation [ @skip-on-coverage ]", async () => {
    ...
    // Check A's balance has decreased by 10 USDD
    assert.equal(A_balanceBefore.sub(await usddToken.balanceOf(A)), dec(10, 18));
  ...
  it("redeemCollateral(): a redemption made at zero base rate send a non-zero ETHFee to DEFT staking contract", async () => {
    // time fast-forwards 1 year, and alice stakes 1 DEFT
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
    await deftToken.approve(deftStaking.address, dec(1, 18), { from: alice });
    await deftStaking.stake(dec(1, 18), { from: alice });
    ...
    // Check DEFT Staking contract balance before is zero
    const deftStakingBalance_Before = await collToken.balanceOf(deftStaking.address);
    assert.equal(deftStakingBalance_Before, "0");
    ...
    // Check DEFT Staking contract balance after is non-zero
    const deftStakingBalance_After = toBN(await collToken.balanceOf(deftStaking.address));
    assert.isTrue(deftStakingBalance_After.gt(toBN("0")));
  });

  it("redeemCollateral(): a redemption made at zero base increases the ETH-fees-per-DEFT-staked in DEFT Staking contract", async () => {
    // time fast-forwards 1 year, and alice stakes 1 DEFT
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
    await deftToken.approve(deftStaking.address, dec(1, 18), { from: alice });
    await deftStaking.stake(dec(1, 18), { from: alice });
    ...
    // Check DEFT Staking ETH-fees-per-DEFT-staked before is zero
    const F_ETH_Before = await deftStaking.F_COLLS(collToken.address);
    ...
    // Check DEFT Staking ETH-fees-per-DEFT-staked after is non-zero
    const F_ETH_After = await deftStaking.F_COLLS(collToken.address);
    assert.isTrue(F_ETH_After.gt("0"));
  });

  it("redeemCollateral(): a redemption made at a non-zero base rate send a non-zero ETHFee to DEFT staking contract", async () => {
    // time fast-forwards 1 year, and alice stakes 1 DEFT
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
    await deftToken.approve(deftStaking.address, dec(1, 18), { from: alice });
    await deftStaking.stake(dec(1, 18), { from: alice });
    ...
    const deftStakingBalance_Before = toBN(await collToken.balanceOf(deftStaking.address));
    ...
    // check DEFT Staking balance has increased
    assert.isTrue(deftStakingBalance_After.gt(deftStakingBalance_Before));
  });

  it("redeemCollateral(): a redemption made at a non-zero base rate increases ETH-per-DEFT-staked in the staking contract", async () => {
    // time fast-forwards 1 year, and alice stakes 1 DEFT
    await th.fastForwardTime(timeValues.SECONDS_IN_ONE_YEAR, web3.currentProvider);
    await deftToken.approve(deftStaking.address, dec(1, 18), { from: alice });
    await deftStaking.stake(dec(1, 18), { from: alice });
  ...
  it("redeemCollateral(): a redemption sends the ETH remainder (ETHDrawn - ETHFee) to the redeemer", async () => {
    ...
    const A_balanceBefore = toBN(await collToken.balanceOf(A));
    ...
    // Check total USDD supply
    const activeUSDD = await activePool.getUSDDDebt();
    const defaultUSDD = await defaultPool.getUSDDDebt();

    const totalUSDDSupply = activeUSDD.add(defaultUSDD);
    th.assertIsApproximatelyEqual(totalUSDDSupply, totalDebt);
    ...
    */
    ...
    const A_balanceAfter = toBN(await collToken.balanceOf(A));
  ...
  it("redeemCollateral(): a full redemption (leaving trove with 0 debt), closes the trove", async () => {
    ...
    const A_balanceBefore = toBN(await collToken.balanceOf(A));
    const B_balanceBefore = toBN(await collToken.balanceOf(B));
    const C_balanceBefore = toBN(await collToken.balanceOf(C));

    // whale redeems 360 USDD.  Expect this to fully redeem A, B, C, and partially redeem D.
  ...
  const redeemCollateral3Full1Partial = async () => {
    ...
    const B_balanceBefore = toBN(await collToken.balanceOf(B));
    const C_balanceBefore = toBN(await collToken.balanceOf(C));
    const D_balanceBefore = toBN(await collToken.balanceOf(D));
    ...
    // whale redeems USDD.  Expect this to fully redeem A, B, C, and partially redeem D.
    await th.redeemCollateral(whale, contracts, redemptionAmount);
    ...
    */
    ...
    const A_balanceAfter = toBN(await collToken.balanceOf(A));
    const B_balanceAfter = toBN(await collToken.balanceOf(B));
    const C_balanceAfter = toBN(await collToken.balanceOf(C));
    const D_balanceAfter = toBN(await collToken.balanceOf(D));
  ...
  it("redeemCollateral(): emits correct debt and coll values in each redeemed trove's TroveUpdated event", async () => {
    ...
    // whale redeems USDD.  Expect this to fully redeem A, B, C, and partially redeem 15 USDD from D.
  ...
  it("redeemCollateral(): a redemption that closes a trove leaves the trove's ETH surplus (collateral - ETH drawn) available for the trove owner to claim", async () => {
    ...
    await assertRevert(
      collSurplusPool.claimColl(A, A, collToken.address),
  ...
  it("getPendingUSDDDebtReward(): Returns 0 if there is no pending USDDDebt reward", async () => {
    // Make some troves
    ...
    const current_L_USDDDebt = await troveManager.L_USDDDebt();
    assert.equal(current_L_USDDDebt, 0);

    const carolSnapshot_L_USDDDebt = (await troveManager.rewardSnapshots(carol))[1];
    assert.equal(carolSnapshot_L_USDDDebt, 0);

    const carol_PendingUSDDDebtReward = await troveManager.getPendingUSDDDebtReward(carol);
    assert.equal(carol_PendingUSDDDebtReward, 0);
  });
  ...
  it("getPendingCollReward(): Returns 0 if there is no pending ETH reward", async () => {
    ...
    // Confirm there are no pending rewards from liquidation
    const current_L_ETH = await troveManager.L_COLL();
    ...
    const carol_PendingETHReward = await troveManager.getPendingCollReward(carol);
  ...
  it("computeICR(): Returns 2^256-1 for ETH:USD = 100, coll = 1 ETH, debt = 100 USDD", async () => {
    const price = dec(100, 18);
    const coll = dec(1, "ether");
    const debt = dec(100, 18);
  ...
  it("computeICR(): returns correct ICR for ETH:USD = 100, coll = 200 ETH, debt = 30 USDD", async () => {
    const price = dec(100, 18);
  ...
  it("computeICR(): returns correct ICR for ETH:USD = 250, coll = 1350 ETH, debt = 127 USDD", async () => {
  ...
  it("computeICR(): returns correct ICR for ETH:USD = 100, coll = 1 ETH, debt = 54321 USDD", async () => {
    const price = dec(100, 18);
    const coll = dec(1, "ether");
  ...
  it("getTroveStatus(): Returns status", async () => {
    ...
    await openTrove({
      ICR: toBN(dec(150, 16)),
      extraUSDDAmount: B_totalDebt,
      extraParams: { from: A }
    });

    // to be able to repay:
    await usddToken.transfer(B, B_totalDebt, { from: A });
