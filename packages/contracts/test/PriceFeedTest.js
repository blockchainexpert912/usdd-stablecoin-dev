...
contract("PriceFeed", async accounts => {
  ...
  const setAddresses = async () => {
    await priceFeed.setAddresses(collToken.address, mockChainlink.address, tellorCaller.address, {
      from: owner
    });
  };
  ...
  describe("PriceFeed internal testing contract", async accounts => {
    ...
    it("should be able to fetchPrice after setPrice, output of former matching input of latter", async () => {
      await priceFeedTestnet.setPrice(collToken.address, dec(100, 18));
      const price = await priceFeedTestnet.getPrice(collToken.address);
      assert.equal(price, dec(100, 18));
    });
  });

  describe("Mainnet PriceFeed setup", async accounts => {
    it("fetchPrice should fail on contract with no chainlink and tellor address set", async () => {
      try {
        const price = await zeroAddressPriceFeed.fetchPrice(collToken.address);
        assert.isFalse(price.receipt.status);
      } catch (err) {
        assert.include(err.message, "PriceFeed: Chainlink and Tellor can not be zero address");
      }
    });

    it("setAddresses should fail whe called by nonOwner", async () => {
      await assertRevert(
        priceFeed.setAddresses(collToken.address, mockChainlink.address, mockTellor.address, {
          from: alice
        }),
  ...
  it("C1 Chainlink working: fetchPrice should return the correct price, taking into account the number of decimal digits on the aggregator", async () => {
    ...
    await priceFeed.fetchPrice(collToken.address);
    let price = await priceFeed.lastGoodPrice(collToken.address);
    ...
    await mockChainlink.setPrice(dec(1, 9));
    await priceFeed.fetchPrice(collToken.address);
    price = await priceFeed.lastGoodPrice(collToken.address);
  ...
  it("C1 Chainlink breaks, Tellor working: fetchPrice should return the correct Tellor price, taking into account Tellor's 6-digit granularity", async () => {
    ...
    const statusBefore = await priceFeed.status(collToken.address);
    ...
    const priceFetchTx = await priceFeed.fetchPrice(collToken.address);
    const statusAfter = await priceFeed.status(collToken.address);
  ...
  it("C1 chainlinkWorking: Chainlink broken by zero latest roundId, Tellor working: switch to usingChainlinkTellorUntrusted", async () => {
    ...
    await priceFeed.setLastGoodPrice(collToken.address, dec(999, 18));
  ...
  it("C1 chainlinkWorking: Chainlink is out of date by <3hrs: return Chainklink price", async () => {
    ...
    const priceFetchTx = await priceFeed.fetchPrice(collToken.address);
    const price = await priceFeed.lastGoodPrice(collToken.address);
  ...
  it("C1 chainlinkWorking: Chainlink price drop of >50%, switch to usingChainlinkTellorUntrusted", async () => {
    await setAddresses();
    priceFeed.setLastGoodPrice(collToken.address, dec(2, 18));
  ...
  it("C1 chainlinkWorking: Chainlink price drop of >50% and Tellor frozen: return last good price", async () => {
    await setAddresses();
    priceFeed.setLastGoodPrice(collToken.address, dec(1200, 18)); // establish a "last good price" from the previous price fetch
  ...
  it("C1 chainlinkWorking: Chainlink is working and Tellor is working - remain on chainlinkWorking", async () => {
    await setAddresses();
    priceFeed.setLastGoodPrice(collToken.address, dec(1200, 18));
  ...
  it("C2 usingTellorChainlinkUntrusted: Tellor breaks by zero price: switch to bothOraclesSuspect", async () => {
    await setAddresses();
    priceFeed.setStatus(collToken.address, 1); // status 1: using Tellor, Chainlink untrusted

    await mockChainlink.setPrevPrice(dec(999, 8));
    await mockChainlink.setPrice(dec(999, 8));

    await priceFeed.setLastGoodPrice(collToken.address, dec(123, 18));
    ...
    const status = await priceFeed.status(collToken.address);
  ...
  it("C2 usingTellorChainlinkUntrusted: Tellor freezes - remain usingChainlinkTellorUntrusted", async () => {
    ...
    await priceFeed.setLastGoodPrice(collToken.address, dec(246, 18));
  ...
  it("C3 bothOraclesUntrusted: both Tellor and Chainlink are live and > 5% price difference remain bothOraclesSuspect", async () => {
    await setAddresses();
    priceFeed.setStatus(collToken.address, 2); // status 2: both oracles untrusted

    await priceFeed.setLastGoodPrice(collToken.address, dec(50, 18));
  ...
  it("C4 usingTellorChainlinkFrozen: when both Chainlink and Tellor break, switch to bothOraclesSuspect", async () => {
    await setAddresses();
    priceFeed.setStatus(collToken.address, 3); // status 3: using Tellor, Chainlink frozen
  ...
  it("C4 usingTellorChainlinkFrozen: when both Chainlink and Tellor break, return last good price", async () => {
    await setAddresses();
    priceFeed.setStatus(collToken.address, 2); // status 2: using tellor, chainlink frozen
  ...
  it("C5 usingChainlinkTellorUntrusted: when Chainlink is live and Tellor price >5% - no status change", async () => {
    await setAddresses();
    priceFeed.setStatus(collToken.address, 4); // status 4: using chainlink, Tellor untrusted
