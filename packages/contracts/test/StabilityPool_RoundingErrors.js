...
contract("Pool Manager: Sum-Product rounding errors", async accounts => {
  ...
  it.skip("Rounding errors: 100 deposits of 100USDD into SP, then 200 liquidations of 49USDD", async () => {
    ...
    const SP_TotalDeposits = await stabilityPool.getTotalUSDDDeposits();
    const SP_ETH = await stabilityPool.getETH();
    const compoundedDeposit = await stabilityPool.getCompoundedUSDDDeposit(depositors[0]);
