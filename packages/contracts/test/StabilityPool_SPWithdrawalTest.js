...
contract("StabilityPool - Withdrawal of stability deposit - Reward calculations", async accounts => {
  ...
  describe("Stability Pool Withdrawal", async () => {
    ...
    beforeEach(async () => {
      ...
      const deftToken = DEFTContracts.deftToken;
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      await openTrove({
        ...
        usddAmount: await getOpenTroveUSDDAmount(dec(100000, 18)),
        upperHint: whale,
        lowerHint: whale,
        extraParams: { from: whale, value: toBN(dec(100000, "ether")) }
      });

      // Whale transfers 10k USDD to A, B and C who then deposit it to the SP
      const depositors = [alice, bob, carol];
      for (account of depositors) {
        await usddToken.transfer(account, dec(10000, 18), { from: whale });
        await stabilityPool.provideToSP(dec(10000, 18), ZERO_ADDRESS, { from: account });
      }

      // Defaulter opens trove with 200% ICR and 10k USDD net debt
      await openTrove({
        maxFeePercentage: th._100pct,
      ...
      // Check depositors' compounded deposit is 6666.66 USDD and ETH Gain is 33.16 ETH
      ...
      const alice_ETHWithdrawn = th
        .getEventArgByName(txA, "CollGainWithdrawn", "_collAmount")
        .toString();
      const bob_ETHWithdrawn = th
        .getEventArgByName(txB, "CollGainWithdrawn", "_collAmount")
        .toString();
      const carol_ETHWithdrawn = th
        .getEventArgByName(txC, "CollGainWithdrawn", "_collAmount")
        .toString();

      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(alice)).toString(), "6666666666666666666666"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(bob)).toString(), "6666666666666666666666"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(carol)).toString(), "6666666666666666666666"),
        10000
      );
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two identical liquidations", async () => {
      ...
      await openTrove({
        ...
        extraParams: { from: defaulter_2, value: toBN(dec(100, "ether")) }
      });
      ...
      // Check depositors' compounded deposit is 3333.33 USDD and ETH Gain is 66.33 ETH
      const txA = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: alice });
      const txB = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: bob });
      const txC = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: carol });
      // Grab the ETH gain from the emitted event in the tx log
      ...
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(alice)).toString(), "3333333333333333333333"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(bob)).toString(), "3333333333333333333333"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(carol)).toString(), "3333333333333333333333"),
        10000
      );
    ...
    it("withdrawFromSP():  Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three identical liquidations", async () => {
      ...
      // Check depositors' compounded deposit is 0 USDD and ETH Gain is 99.5 ETH
      const txA = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: alice });
      const txB = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: bob });
      const txC = await stabilityPool.withdrawFromSP(dec(10000, 18), { from: carol });
      ...
      assert.isAtMost(th.getDifference((await usddToken.balanceOf(alice)).toString(), "0"), 10000);
      assert.isAtMost(th.getDifference((await usddToken.balanceOf(bob)).toString(), "0"), 10000);
      assert.isAtMost(th.getDifference((await usddToken.balanceOf(carol)).toString(), "0"), 10000);
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing USDD", async () => {
      // Whale opens Trove with 100k ETH
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(100000, 18)),
      ...
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(5000, 18)),
        upperHint: defaulter_1,
        lowerHint: defaulter_1,
        extraParams: { from: defaulter_1, value: toBN(dec(50, "ether")) }
      });
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(7000, 18)),
        upperHint: defaulter_2,
        lowerHint: defaulter_2,
        extraParams: { from: defaulter_2, value: toBN(dec(70, "ether")) }
      });
      ...
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(alice)).toString(), "6000000000000000000000"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(bob)).toString(), "6000000000000000000000"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(carol)).toString(), "6000000000000000000000"),
        10000
      );
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing USDD", async () => {
      // Whale opens Trove with 100k ETH
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(100000, 18)),
      ...
      await openTrove({
        ...
        upperHint: defaulter_2,
        lowerHint: defaulter_2,
        extraParams: { from: defaulter_2, value: toBN(dec(60, "ether")) }
      });
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(7000, 18)),
        upperHint: defaulter_3,
        lowerHint: defaulter_3,
        extraParams: { from: defaulter_3, value: toBN(dec(70, "ether")) }
      });
      ...
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(alice)).toString(), "4000000000000000000000"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(bob)).toString(), "4000000000000000000000"),
        10000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(carol)).toString(), "4000000000000000000000"),
        10000
      );
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after two identical liquidations", async () => {
      ...
      // Whale transfers 10k, 20k, 30k USDD to A, B and C respectively who then deposit it to the SP
      ...
      await usddToken.transfer(bob, dec(20000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(20000, 18), ZERO_ADDRESS, { from: bob });
      await usddToken.transfer(carol, dec(30000, 18), { from: whale });
      ...
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(bob)).toString(), "13333333333333333333333"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(carol)).toString(), "20000000000000000000000"),
        100000
      );
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three identical liquidations", async () => {
      ...
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(alice)).toString(), "5000000000000000000000"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(bob)).toString(), "10000000000000000000000"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(carol)).toString(), "15000000000000000000000"),
        100000
      );
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three varying liquidations", async () => {
      ...
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(1000000, 18)),
        upperHint: whale,
        lowerHint: whale,
        extraParams: { from: whale, value: toBN(dec(1000000, "ether")) }
      });
      ...
      Bob:  456000 USDD
      Carol: 13100 USDD */
      // Whale transfers USDD to  A, B and C respectively who then deposit it to the SP
      await usddToken.transfer(alice, dec(2000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(2000, 18), ZERO_ADDRESS, { from: alice });
      await usddToken.transfer(bob, dec(456000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(456000, 18), ZERO_ADDRESS, { from: bob });
      await usddToken.transfer(carol, dec(13100, 18), { from: whale });
      await stabilityPool.provideToSP(dec(13100, 18), ZERO_ADDRESS, { from: carol });

      /* Defaulters open troves
     
      Defaulter 1: 207000 USDD & 2160 ETH
      Defaulter 2: 5000 USDD & 50 ETH
      Defaulter 3: 46700 USDD & 500 ETH
      */
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(207000, 18)),
        upperHint: defaulter_1,
        lowerHint: defaulter_1,
        extraParams: { from: defaulter_1, value: toBN(dec(2160, "ether")) }
      });
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(5000, 18)),
        upperHint: defaulter_2,
        lowerHint: defaulter_2,
        extraParams: { from: defaulter_2, value: toBN(dec(50, "ether")) }
      });
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(46700, 18)),
        upperHint: defaulter_3,
        lowerHint: defaulter_3,
        extraParams: { from: defaulter_3, value: toBN(dec(500, "ether")) }
      });
      ...
      // ()
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(alice)).toString(), "901719380174061000000"),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(bob)).toString(), "205592018679686000000000"),
        10000000000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(carol)).toString(), "5906261940140100000000"),
        10000000000
      );
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 USDD.  A, B, C, D withdraw correct USDD deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(100000, 18)),
      ...
      await usddToken.transfer(dennis, dec(10000, 18), { from: whale });
      ...
      const dennis_ETHWithdrawn = th
        .getEventArgByName(txD, "CollGainWithdrawn", "_collAmount")
        .toString();

      console.log();
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(alice)).toString(), "1666666666666666666666"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(bob)).toString(), "1666666666666666666666"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(carol)).toString(), "1666666666666666666666"),
        100000
      );

      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(dennis)).toString(), "5000000000000000000000"),
        100000
      );
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 USDD.  A, B, C, D withdraw correct USDD deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(100000, 18)),
      ...
      await openTrove({
        ...
        extraParams: { from: defaulter_4, value: toBN(dec(100, "ether")) }
      });
      ...
      assert.isAtMost(th.getDifference((await usddToken.balanceOf(alice)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usddToken.balanceOf(bob)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usddToken.balanceOf(carol)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usddToken.balanceOf(dennis)).toString(), "0"), 100000);
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct USDD deposit and ETH Gain", async () => {
      // Whale opens Trove with 1m ETH
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(1000000, 18)),
      ...
      */
      // Whale transfers USDD to  A, B and C respectively who then deposit it to the SP
      await usddToken.transfer(alice, dec(60000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(60000, 18), ZERO_ADDRESS, { from: alice });
      await usddToken.transfer(bob, dec(20000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(20000, 18), ZERO_ADDRESS, { from: bob });
      await usddToken.transfer(carol, dec(15000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(15000, 18), ZERO_ADDRESS, { from: carol });

      /* Defaulters open troves:
      Defaulter 1:  10000 USDD, 100 ETH
      Defaulter 2:  25000 USDD, 250 ETH
      Defaulter 3:  5000 USDD, 50 ETH
      Defaulter 4:  40000 USDD, 400 ETH
      */
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(10000, 18)),
      ...
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(25000, 18)),
        upperHint: defaulter_2,
        lowerHint: defaulter_2,
        extraParams: { from: defaulter_2, value: toBN(dec(250, "ether")) }
      });
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(5000, 18)),
        upperHint: defaulter_3,
        lowerHint: defaulter_3,
        extraParams: { from: defaulter_3, value: toBN(dec(50, "ether")) }
      });
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(40000, 18)),
        upperHint: defaulter_4,
        lowerHint: defaulter_4,
        extraParams: { from: defaulter_4, value: toBN(dec(400, "ether")) }
      });
      ...
      // Dennis provides 25000 USDD
      await usddToken.transfer(dennis, dec(25000, 18), { from: whale });
      ...
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(alice)).toString(), "17832817337461300000000"),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(bob)).toString(), "5944272445820430000000"),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(carol)).toString(), "4458204334365320000000"),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(dennis)).toString(), "11764705882352900000000"),
        100000000000
      );
    ...
    it("withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 USDD.  A, B, C, D withdraw correct USDD deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(100000, 18)),
      ...
      assert.isAtMost(th.getDifference((await usddToken.balanceOf(alice)).toString(), "0"), 1000);
      assert.isAtMost(th.getDifference((await usddToken.balanceOf(bob)).toString(), "0"), 1000);
      assert.isAtMost(th.getDifference((await usddToken.balanceOf(carol)).toString(), "0"), 1000);
    ...
    it("withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct USDD deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(100000, 18)),
      ...
      Dennis: 40000 USDD
      */
      // Whale transfers USDD to  A, B,C and D respectively who then deposit it to the SP
      await usddToken.transfer(alice, dec(20000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(20000, 18), ZERO_ADDRESS, { from: alice });
      await usddToken.transfer(bob, dec(25000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(25000, 18), ZERO_ADDRESS, { from: bob });
      await usddToken.transfer(carol, dec(12500, 18), { from: whale });
      await stabilityPool.provideToSP(dec(12500, 18), ZERO_ADDRESS, { from: carol });
      await usddToken.transfer(dennis, dec(40000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(40000, 18), ZERO_ADDRESS, { from: dennis });

      /* Defaulters open troves:
      Defaulter 1: 10000 USDD
      Defaulter 2: 20000 USDD
      Defaulter 3: 30000 USDD
      Defaulter 4: 5000 USDD
      */
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(10000, 18)),
      ...
      await openTrove({
        ...
        upperHint: defaulter_3,
        lowerHint: defaulter_3,
        extraParams: { from: defaulter_3, value: toBN(dec(300, "ether")) }
      });
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(5000, 18)),
        upperHint: defaulter_4,
        lowerHint: defaulter_4,
        extraParams: { from: defaulter_4, value: toBN(dec(50, "ether")) }
      });
      ...
      const dennis_ETHWithdrawn = th
        ...
        .toString();
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(dennis)).toString(), "27692307692307700000000"),
        100000000000
      );
      ...
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(alice)).toString(), "1672240802675590000000"),
        10000000000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(bob)).toString(), "2090301003344480000000"),
        100000000000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(carol)).toString(), "1045150501672240000000"),
        100000000000
      );
    ...
    it("withdrawFromSP(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 USDD. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct USDD deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(100000, 18)),
      ...
      // Whale transfers 10k USDD to A, B and D who then deposit it to the SP
      ...
      // Carol makes deposit
      await usddToken.transfer(carol, dec(10000, 18), { from: whale });
      ...
      const dennis_ETHWithdrawn = th
        ...
        .toString();
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(dennis)).toString(), "1666666666666666666666"),
        100000
      );
      ...
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(alice)).toString(), "666666666666666666666"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(bob)).toString(), "666666666666666666666"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(carol)).toString(), "2000000000000000000000"),
        100000
      );
    ...
    // A, B withdraw 0USDD & 100e
    // C, D withdraw 5000USDD  & 500e
    it("withdrawFromSP(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
      ...
      // Whale transfers 10k USDD to A, B who then deposit it to the SP
      ...
      // Defaulter 1 liquidated. 20000 USDD fully offset with pool.
      await troveManager.liquidate(defaulter_1, { from: owner });

      // Carol, Dennis each deposit 10000 USDD
      ...
      // Defaulter 2 liquidated. 10000 USDD offset
      await troveManager.liquidate(defaulter_2, { from: owner });
      ...
      // Expect Alice And Bob's compounded deposit to be 0 USDD
      assert.isAtMost(th.getDifference((await usddToken.balanceOf(alice)).toString(), "0"), 10000);
      assert.isAtMost(th.getDifference((await usddToken.balanceOf(bob)).toString(), "0"), 10000);
      ...
      // Expect Carol And Dennis' compounded deposit to be 50 USDD
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(carol)).toString(), "5000000000000000000000"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(dennis)).toString(), "5000000000000000000000"),
    ...
    it("withdrawFromSP(): Pool-emptying liquidation increases epoch by one, resets scaleFactor to 0, and resets P to 1e18", async () => {
      ...
      // Defaulter 1 liquidated. 10--0 USDD fully offset, Pool remains non-zero
      await troveManager.liquidate(defaulter_1, { from: owner });
      ...
      // Defaulter 2 liquidated. 1--00 USDD, empties pool
      await troveManager.liquidate(defaulter_2, { from: owner });
      ...
      // Defaulter 3 liquidated. 10000 USDD fully offset, Pool remains non-zero
      await troveManager.liquidate(defaulter_3, { from: owner });
      ...
      // Defaulter 4 liquidated. 10000 USDD, empties pool
      await troveManager.liquidate(defaulter_4, { from: owner });
    ...
    // A, B withdraw 0 USDD & 100e
    // C, D withdraw 5000 USDD  & 50e
    it("withdrawFromSP(): Depositors withdraw correct compounded deposit after liquidation empties the pool", async () => {
      ...
      // Carol, Dennis, Erin each deposit 10000, 20000, 30000 USDD respectively
      await usddToken.transfer(carol, dec(10000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(10000, 18), ZERO_ADDRESS, { from: carol });

      await usddToken.transfer(dennis, dec(20000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(20000, 18), ZERO_ADDRESS, { from: dennis });

      await usddToken.transfer(erin, dec(30000, 18), { from: whale });
      ...
      const erin_ETHWithdrawn = th
        .getEventArgByName(txE, "CollGainWithdrawn", "_collAmount")
        .toString();
      ...
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(carol)).toString(), "8333333333333333333333"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(dennis)).toString(), "16666666666666666666666"),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(erin)).toString(), "25000000000000000000000"),
        100000
      );
    ...
    // A deposits 10000
    // L1, L2, L3 liquidated with 10000 USDD each
    // A withdraws all
    ...
    it("withdrawFromSP(): single deposit fully offset. After subsequent liquidations, depositor withdraws 0 deposit and *only* the ETH Gain from one liquidation", async () => {
      ...
      // Defaulter 1,2,3 withdraw 10000 USDD
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(10000, 18)),
        upperHint: defaulter_1,
    ...
    // A,B deposit 10000 USDD
    // L1 cancels 20000 USDD, 2E
    // B,C deposits 10000 USDD
    // L2 cancels 20000 USDD, 2E
    // E,F deposit 10000 USDD
    // L3 cancels 20000, 200E
    // G,H deposits 10000
    // L4 cancels 20000, 200E

    // Expect all depositors withdraw 0 USDD and 100 ETH
    ...
    it("withdrawFromSP(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
      ...
      // Alice, Bob each deposit 10k USDD
      ...
      // Erin, Flyn each deposit 10000 USDD
      ...
      // Defaulter 3 liquidated. 10000 USDD offset
      await troveManager.liquidate(defaulter_3, { from: owner });

      // Graham, Harriet each deposit 10000 USDD
      ...
      // Defaulter 4 liquidated. 10k USDD offset
      await troveManager.liquidate(defaulter_4, { from: owner });
      ...
      const flyn_ETHWithdrawn = th
        .getEventArgByName(txF, "CollGainWithdrawn", "_collAmount")
        .toString();
      const graham_ETHWithdrawn = th
        .getEventArgByName(txG, "CollGainWithdrawn", "_collAmount")
        .toString();
      const harriet_ETHWithdrawn = th
        .getEventArgByName(txH, "CollGainWithdrawn", "_collAmount")
        .toString();

      // Expect all deposits to be 0 USDD
      assert.isAtMost(th.getDifference((await usddToken.balanceOf(alice)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usddToken.balanceOf(bob)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usddToken.balanceOf(carol)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usddToken.balanceOf(dennis)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usddToken.balanceOf(erin)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usddToken.balanceOf(flyn)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usddToken.balanceOf(graham)).toString(), "0"), 100000);
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(harriet)).toString(), "0"),
        100000
      );
    ...
    // A withdraws all
    // B deposits 10000
    // L2 of 9900 USDD, should bring P slightly past boundary i.e. 1e-9 -> 1e-10
    ...
    it("withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Defaulter 1 withdraws 'almost' 10000 USDD:  9999.99991 USDD
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount("9999999910000000000000"),
        upperHint: defaulter_1,
        lowerHint: defaulter_1,
        extraParams: { from: defaulter_1, value: toBN(dec(100, "ether")) }
      });
      ...
      // Defaulter 2 withdraws 9900 USDD
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(9900, 18)),
        upperHint: defaulter_2,
        lowerHint: defaulter_2,
        extraParams: { from: defaulter_2, value: toBN(dec(60, "ether")) }
      });
      ...
      // Defaulter 2 liquidated.  9900 USDD liquidated. P altered by a factor of 1-(9900/10000) = 0.01.  Scale changed.
      await troveManager.liquidate(defaulter_2, { from: owner });
      ...
      // Expect Bob to withdraw 1% of initial deposit (100 USDD) and all the liquidated ETH (60 ether)
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(bob)).toString(), "100000000000000000000"),
        100000
      );
    ...
    // A deposits 10000
    // L1 brings P close to boundary, i.e. 9e-9: liquidate 9999.99991 USDD
    // A withdraws all
    ...
    it("withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Defaulter 1 withdraws 'almost' 10k USDD.
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount("9999999910000000000000"),
        upperHint: defaulter_1,
      ...
      // Defaulter 2 withdraws 59400 USDD
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount("59400000000000000000000"),
        upperHint: defaulter_2,
        lowerHint: defaulter_2,
        extraParams: { from: defaulter_2, value: toBN(dec(330, "ether")) }
      });
      ...
      await usddToken.transfer(carol, dec(20000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(20000, 18), ZERO_ADDRESS, { from: carol });

      await usddToken.transfer(dennis, dec(30000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(30000, 18), ZERO_ADDRESS, { from: dennis });

      // 54000 USDD liquidated.  P altered by a factor of 1-(59400/60000) = 0.01. Scale changed.
      ...
      Bob:  1000 USDD, 55 Ether
      Carol:  2000 USDD, 110 Ether
      Dennis:  3000 USDD, 165 Ether
     
      Total: 6000 USDD, 300 Ether
      */
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(bob)).toString(), dec(100, 18)),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(carol)).toString(), dec(200, 18)),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(dennis)).toString(), dec(300, 18)),
        100000
      );
    ...
    // A make deposit 10000 USDD
    // L1 brings P to 1e-5*P. L1:  9999.9000000000000000 USDD
    // A withdraws
    // B makes deposit 10000 USDD
    // L2 decreases P again by 1e-5, over the scale boundary: 9999.9000000000000000 (near to the 10000 USDD total deposits)
    // B withdraws
    ...
    it("withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Defaulter 1 and default 2 each withdraw 9999.999999999 USDD
      await openTrove({
        maxFeePercentage: th._100pct,
      ...
      // Bob should withdraw 1e-5 of initial deposit: 0.1 USDD and the full ETH gain of 100 ether
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(bob)).toString(), dec(1, 17)),
        100000
      );
    ...
    // L2 decreases P again by 1e-5, over boundary. L2: 59999.4000000000000000  (near to the 60000 USDD total deposits)
    // B withdraws
    // expect d(B) = d0(B) * 1e-5
    // expect B gets entire ETH gain from L2
    it("withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Defaulter 1 and default 2 withdraw up to debt of 9999.9 USDD and 59999.4 USDD
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(99999, 17)),
        upperHint: defaulter_1,
      ...
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(599994, 17)),
        upperHint: defaulter_2,
        lowerHint: defaulter_2,
        extraParams: { from: defaulter_2, value: toBN(dec(600, "ether")) }
      });
      ...
      // B, C, D deposit 10000, 20000, 30000 USDD
      await usddToken.transfer(bob, dec(10000, 18), { from: whale });
      await stabilityPool.provideToSP(dec(10000, 18), ZERO_ADDRESS, { from: bob });
      ...
      // {B, C, D} should have a compounded deposit of {0.1, 0.2, 0.3} USDD
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(bob)).toString(), dec(1, 17)),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(carol)).toString(), dec(2, 17)),
        100000
      );
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(dennis)).toString(), dec(3, 17)),
        100000
      );
    ...
    // A make deposit 10000 USDD
    // L1 brings P to (~1e-10)*P. L1: 9999.9999999000000000 USDD
    ...
    it("withdrawFromSP(): Deposit that decreases to less than 1e-9 of it's original value is reduced to 0", async () => {
      ...
      // Defaulters 1 withdraws 9999.9999999 USDD
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount("9999999999900000000000"),
        upperHint: defaulter_1,
        lowerHint: defaulter_1,
        extraParams: { from: defaulter_1, value: toBN(dec(100, "ether")) }
      });
      ...
      const aliceDeposit = (await stabilityPool.getCompoundedUSDDDeposit(alice)).toString();
    ...
    /* A make deposit 10000 USDD
    L1 brings P to 0.0001P. L1:  9999.900000000000000000 USDD, 1 ETH
    B makes deposit 9999.9, brings SP to 10k
    L2 decreases P by(~1e-5)P. L2:  9999.900000000000000000 USDD, 1 ETH
    C makes deposit 9999.9, brings SP to 10k
    L3 decreases P by(~1e-5)P. L3:  9999.900000000000000000 USDD, 1 ETH
    D makes deposit 9999.9, brings SP to 10k
    L4 decreases P by(~1e-5)P. L4:  9999.900000000000000000 USDD, 1 ETH
    expect A, B, C, D each withdraw ~100 Ether
    */
    it("withdrawFromSP(): Several deposits of 10000 USDD span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      // Whale opens Trove with 100k ETH
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(100000, 18)),
      ...
      // Defaulters 1-4 each withdraw 9999.9 USDD
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(99999, 17)),
        upperHint: defaulter_1,
      ...
      // B deposits 9999.9 USDD
      await usddToken.transfer(bob, dec(99999, 17), { from: whale });
      ...
      // C deposits 9999.9 USDD
      await usddToken.transfer(carol, dec(99999, 17), { from: whale });
      ...
      // D deposits 9999.9 USDD
      await usddToken.transfer(dennis, dec(99999, 17), { from: whale });
      ...
      assert.equal(await usddToken.balanceOf(alice), "0");
      assert.equal(await usddToken.balanceOf(alice), "0");
      assert.equal(await usddToken.balanceOf(alice), "0");
      // D should withdraw around 0.9999 USDD, since his deposit of 9999.9 was reduced by a factor of 1e-5
      assert.isAtMost(
        th.getDifference((await usddToken.balanceOf(dennis)).toString(), dec(99999, 12)),
        100000
      );
    ...
    it("withdrawFromSP(): 2 depositors can withdraw after each receiving half of a pool-emptying liquidation", async () => {
      ...
      // Defaulters 1-3 each withdraw 24100, 24300, 24500 USDD (inc gas comp)
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(24100, 18)),
        upperHint: defaulter_1,
        lowerHint: defaulter_1,
        extraParams: { from: defaulter_1, value: toBN(dec(200, "ether")) }
      });
      ...
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(24300, 18)),
        upperHint: defaulter_2,
        lowerHint: defaulter_2,
        extraParams: { from: defaulter_2, value: toBN(dec(200, "ether")) }
      });
      ...
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(24500, 18)),
        upperHint: defaulter_3,
        lowerHint: defaulter_3,
        extraParams: { from: defaulter_3, value: toBN(dec(200, "ether")) }
      });
      ...
      // A, B provide 10k USDD
      await usddToken.transfer(A, dec(10000, 18), { from: whale });
      await usddToken.transfer(B, dec(10000, 18), { from: whale });
      ...
      const A_deposit = await stabilityPool.getCompoundedUSDDDeposit(A);
      const B_deposit = await stabilityPool.getCompoundedUSDDDeposit(B);
      ...
      const USDDinSP_1 = await stabilityPool.getTotalUSDDDeposits();
      // console.log(`USDDinSP_1: ${USDDinSP_1}`)
      assert.equal(USDDinSP_1, "0");

      // Check SP USDD balance is zero
      const SPUSDDBalance_1 = await usddToken.balanceOf(stabilityPool.address);
      // console.log(`SPUSDDBalance_1: ${SPUSDDBalance_1}`)
      assert.equal(SPUSDDBalance_1, "0");
      ...
      // C, D provide 10k USDD
      await usddToken.transfer(C, dec(10000, 18), { from: whale });
      await usddToken.transfer(D, dec(10000, 18), { from: whale });
      ...
      // Check compounded deposits
      const C_deposit = await stabilityPool.getCompoundedUSDDDeposit(C);
      const D_deposit = await stabilityPool.getCompoundedUSDDDeposit(D);
      ...
      // Check SP tracker is zero
      const USDDinSP_2 = await stabilityPool.getTotalUSDDDeposits();
      // console.log(`USDDinSP_2: ${USDDinSP_2}`)
      assert.equal(USDDinSP_2, "0");

      // Check SP USDD balance is zero
      const SPUSDDBalance_2 = await usddToken.balanceOf(stabilityPool.address);
      // console.log(`SPUSDDBalance_2: ${SPUSDDBalance_2}`)
      assert.equal(SPUSDDBalance_2, "0");
      ...
      // E, F provide 10k USDD
      await usddToken.transfer(E, dec(10000, 18), { from: whale });
      await usddToken.transfer(F, dec(10000, 18), { from: whale });
      ...
      // Check compounded deposits
      const E_deposit = await stabilityPool.getCompoundedUSDDDeposit(E);
      const F_deposit = await stabilityPool.getCompoundedUSDDDeposit(F);
      ...
      // Check SP tracker is zero
      const USDDinSP_3 = await stabilityPool.getTotalUSDDDeposits();
      assert.equal(USDDinSP_3, "0");

      // Check SP USDD balance is zero
      const SPUSDDBalance_3 = await usddToken.balanceOf(stabilityPool.address);
      // console.log(`SPUSDDBalance_3: ${SPUSDDBalance_3}`)
      assert.equal(SPUSDDBalance_3, "0");
    ...
    it("withdrawFromSP(): Depositor's ETH gain stops increasing after two scale changes", async () => {
      ...
      // Defaulters 1-5 each withdraw up to debt of 9999.9999999 USDD
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(99999, 17)),
        upperHint: defaulter_1,
      ...
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(99999, 17)),
        upperHint: defaulter_5,
        lowerHint: defaulter_5,
        extraParams: { from: defaulter_5, value: toBN(dec(100, "ether")) }
      });
      ...
      // E deposits 9999.9 USDD
      await usddToken.transfer(erin, dec(99999, 17), { from: whale });
    ...
    it("withdrawFromSP(): Large liquidated coll/debt, deposits and ETH price", async () => {
      ...
      for (account of depositors) {
        await openTrove({
          maxFeePercentage: th._100pct,
          usddAmount: toBN(dec(1, 36)),
          upperHint: account,
          lowerHint: account,
          extraParams: { from: account, value: toBN(dec(2, 27)) }
        });
      ...
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(1, 36)),
        upperHint: defaulter_1,
        lowerHint: defaulter_1,
        extraParams: { from: defaulter_1, value: toBN(dec(1, 27)) }
      });
      ...
      // Grab the ETH gain from the emitted event in the tx log
      const alice_ETHWithdrawn = th.getEventArgByName(txA, "CollGainWithdrawn", "_collAmount");
      const bob_ETHWithdrawn = th.getEventArgByName(txB, "CollGainWithdrawn", "_collAmount");

      // Check USDD balances
      const aliceUSDDBalance = await usddToken.balanceOf(alice);
      const aliceExpectedUSDDBalance = web3.utils.toBN(dec(5, 35));
      const aliceUSDDBalDiff = aliceUSDDBalance.sub(aliceExpectedUSDDBalance).abs();

      assert.isTrue(aliceUSDDBalDiff.lte(toBN(dec(1, 18)))); // error tolerance of 1e18

      const bobUSDDBalance = await usddToken.balanceOf(bob);
      const bobExpectedUSDDBalance = toBN(dec(5, 35));
      const bobUSDDBalDiff = bobUSDDBalance.sub(bobExpectedUSDDBalance).abs();

      assert.isTrue(bobUSDDBalDiff.lte(toBN(dec(1, 18))));
    ...
    it("withdrawFromSP(): Small liquidated coll/debt, large deposits and ETH price", async () => {
      ...
      for (account of depositors) {
        await openTrove({
          maxFeePercentage: th._100pct,
          usddAmount: toBN(dec(1, 38)),
          upperHint: account,
          lowerHint: account,
          extraParams: { from: account, value: toBN(dec(2, 29)) }
        });
        await stabilityPool.provideToSP(dec(1, 38), ZERO_ADDRESS, { from: account });
      }

      // Defaulter opens trove with 50e-7 ETH and  5000 USDD. 200% ICR
      await openTrove({
        maxFeePercentage: th._100pct,
        usddAmount: await getOpenTroveUSDDAmount(dec(5000, 18)),
        upperHint: defaulter_1,
        lowerHint: defaulter_1,
        extraParams: { from: defaulter_1, value: toBN(dec("5000000000000")) }
      });
      ...
      const aliceUSDDBalance = await usddToken.balanceOf(alice);
      const aliceExpectedUSDDBalance = toBN("99999999999999997500000000000000000000");
      const aliceUSDDBalDiff = aliceUSDDBalance.sub(aliceExpectedUSDDBalance).abs();

      assert.isTrue(aliceUSDDBalDiff.lte(toBN(dec(1, 18))));

      const bobUSDDBalance = await usddToken.balanceOf(bob);
      const bobExpectedUSDDBalance = toBN("99999999999999997500000000000000000000");
      const bobUSDDBalDiff = bobUSDDBalance.sub(bobExpectedUSDDBalance).abs();

      assert.isTrue(bobUSDDBalDiff.lte(toBN("100000000000000000000")));
