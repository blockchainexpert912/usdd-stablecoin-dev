...
const Lqty = artifacts.require("ERC20Mock");
...
contract("Unipool", function ([_, wallet1, wallet2, wallet3, wallet4, bountyAddress, owner]) {
  ...
  const deploy = async that => {
    that.uni = await Uni.new("Uniswap token", "LPT");
    that.pool = await Unipool.new();

    const communityIssuance = await NonPayable.new();
    const deftStaking = await NonPayable.new();
    const lockupContractFactory = await NonPayable.new();
    that.deft = await Lqty.new("DEFT", "DEFT");
    that.lpRewardsEntitlement = new BN((1000000 * 4) / 3).mul(_1e18);
  ...
  describe("Unipool", async function () {
    beforeEach(async function () {
      await deploy(this);
      await this.pool.setAddresses(this.deft.address, this.uni.address);
      await this.deft.mint(this.pool.address, this.lpRewardsEntitlement);
      await this.pool.start(this.lpRewardsEntitlement, this.DURATION);
    });
    ...
    it("Three stakers with the different (1:3:5) stakes wait different durations", async function () {
      ...
      expect(await this.pool.earned(wallet2)).to.be.bignumber.equal("0");
      expect(await this.deft.balanceOf(wallet2)).to.be.bignumber.almostEqualDiv1e18(
    ...
    it("Four stakers with gaps of zero total supply", async function () {
      ...
      expect(await this.pool.earned(wallet1)).to.be.bignumber.equal("0");
      expect(await this.deft.balanceOf(wallet1)).to.be.bignumber.almostEqualDiv1e18(
        rewardPerToken1.add(rewardPerToken2).mul(stake1).div(_1e18)
      );
      expect(await this.pool.earned(wallet2)).to.be.bignumber.almostEqualDiv1e18(
        rewardPerToken2.mul(stake2).div(_1e18)
      ...
      expect(await this.pool.earned(wallet2)).to.be.bignumber.equal("0");
      expect(await this.pool.earned(wallet3)).to.be.bignumber.equal("0");
      expect(await this.deft.balanceOf(wallet3)).to.be.bignumber.almostEqualDiv1e18(
