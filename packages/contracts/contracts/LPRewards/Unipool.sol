...
/*
 * On deployment a new Uniswap pool will be created for the pair USDD/ETH and its token will be set here.
 ...
 * Funds for rewards will only be added once, on deployment of DEFT token,
 ...
 * in order to avoid getting DEFT tokens locked.
 * That also means that the start time for the program will be the event that occurs first:
 * either DEFT token contract is deployed, and therefore DEFT tokens are minted to Unipool contract,
...
contract Unipool is LPTokenWrapper, Ownable, CheckContract, IUnipool {
    ...
    modifier shouldStarted() {
        require(periodFinish > 0, "Issuance not started");
        _;
    }

    // initialization function
    function setAddresses(address _deftTokenAddress, address _uniTokenAddress)
        external
        override
        onlyOwner
    {
    ...
    function start(uint256 _reward, uint256 _duration) external override onlyOwner {
        assert(_reward > 0);
        assert(_reward <= deftToken.balanceOf(address(this)));
    ...
    function stake(uint256 amount) public override shouldStarted {
    ...
    function withdraw(uint256 amount) public override shouldStarted {
    ...
    function withdrawAndClaim() external override shouldStarted {
        withdraw(balanceOf(msg.sender));
        claimReward();
    }

    function claimReward() public override shouldStarted {
        require(address(uniToken) != address(0), "Liquidity Pool Token has not been set yet");
        ...
        deftToken.transfer(msg.sender, reward);
