...
contract("Echidna debugger", async accounts => {
  ...
  before(async () => {
    ...
    usddToken = await USDDToken.at(await echidnaTester.usddToken());
  ...
  it.only("USDD balance", async () => {
    await echidnaTester.openTroveExt("0", "0", "4210965169908805439447313562489173090");

    const totalSupply = await usddToken.totalSupply();
    const gasPoolBalance = await usddToken.balanceOf(GAS_POOL_ADDRESS);
    const activePoolBalance = await activePool.getUSDDDebt();
    const defaultPoolBalance = await defaultPool.getUSDDDebt();
    const stabilityPoolBalance = await stabilityPool.getTotalUSDDDeposits();
    const currentTrove = await echidnaTester.echidnaProxies(0);
    const troveBalance = usddToken.balanceOf(currentTrove);
