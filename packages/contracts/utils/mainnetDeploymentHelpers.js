...
class MainnetDeploymentHelper {
  ...
  async deployLiquityCoreMainnet(tellorMasterAddr, deploymentState) {
    ...
    const usddTokenFactory = await this.getFactory("USDDToken");
    ...
    const usddTokenParams = [
      troveManager.address,
      stabilityPool.address,
      borrowerOperations.address
    ];
    const usddToken = await this.loadOrDeploy(
      usddTokenFactory,
      "usddToken",
      deploymentState,
      usddTokenParams
    ...
    if (!this.configParams.ETHERSCAN_BASE_URL) {
      ...
      await this.verifyContract("usddToken", deploymentState, usddTokenParams);
    }
  ...
  async deployDEFTContractsMainnet(deploymentState, deftToken) {
    const communityIssuanceFactory = await this.getFactory("CommunityIssuance");
    const deftStakingFactory = await this.getFactory("DEFTStaking");

    const deftStaking = await this.loadOrDeploy(deftStakingFactory, "deftStaking", deploymentState);
    ...
    if (!this.configParams.ETHERSCAN_BASE_URL) {
      ...
      await this.verifyContract("deftStaking", deploymentState);
      await this.verifyContract("communityIssuance", deploymentState);
      await this.verifyContract("deftToken", deploymentState, deftTokenParams);
    }
  ...
  async connectCoreContractsMainnet(contracts, DEFTContracts, chainlinkProxyAddress) {
  ...
  async connectDEFTContractsToCoreMainnet(DEFTContracts, coreContracts) {
    const gasPrice = this.configParams.GAS_PRICE;
    (await this.isOwnershipRenounced(DEFTContracts.deftStaking)) ||
      (await this.sendAndWaitForTransaction(
        DEFTContracts.deftStaking.setAddresses(
          DEFTContracts.deftToken.address,
    ...
    (await this.isOwnershipRenounced(DEFTContracts.communityIssuance)) ||
      (await this.sendAndWaitForTransaction(
        DEFTContracts.communityIssuance.setAddresses(
          DEFTContracts.deftToken.address,
  ...
  async connectUnipoolMainnet(uniPool, DEFTContracts, USDDWETHPairAddr) {
    const gasPrice = this.configParams.GAS_PRICE;
    (await this.isOwnershipRenounced(uniPool)) ||
      (await this.sendAndWaitForTransaction(
        uniPool.setParams(DEFTContracts.deftToken.address, USDDWETHPairAddr, { gasPrice })
      ));
  }
