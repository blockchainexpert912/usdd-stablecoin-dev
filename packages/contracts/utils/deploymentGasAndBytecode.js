...
const DEFTStaking = artifacts.require("./DEFT/DEFTStaking.sol");
const DEFTToken = artifacts.require("./DEFT/DEFTToken.sol");
const CommunityIssuance = artifacts.require("./DEFT/CommunityIssuance.sol");
const HintHelpers = artifacts.require("./HintHelpers.sol");

const CommunityIssuanceTester = artifacts.require("./DEFT/CommunityIssuanceTester.sol");
...
const DEFTContractABIs = [DEFTStaking, DEFTToken, CommunityIssuance];
...
async function main() {
  ...
  const DEFTContracts = await dh.deployDEFTContractsHardhat(ARBITRARY_ADDRESS, ARBITRARY_ADDRESS);
  const testerContracts = await dh.deployTesterContractsHardhat();

  await dh.connectCoreContracts(coreContracts, DEFTContracts);
  await dh.connectDEFTContracts(DEFTContracts);
  await dh.connectDEFTContractsToCore(DEFTContracts, coreContracts);

  console.log(`\n`);
  console.log(`DEFT CONTRACTS`);
  await logContractDeploymentCosts(DEFTContracts);
  console.log(`\n`);
  logContractBytecodeLengths(DEFTContractABIs);
  console.log(`\n`);
