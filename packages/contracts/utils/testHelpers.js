...
class TestHelper {
  ...
  // Adds the gas compensation (50 USDD)
  ...
  /*
   * given the requested USDD amomunt in openTrove, returns the total debt
   * So, it adds the gas compensation and the borrowing fee
   */
  static async getOpenTroveTotalDebt(contracts, usddAmount) {
    const fee = await contracts.troveManager.getBorrowingFee(usddAmount);
    const compositeDebt = await this.getCompositeDebt(contracts, usddAmount);
    return compositeDebt.add(fee);
  }

  /*
   * given the desired total debt, returns the USDD amount that needs to be requested in openTrove
   * So, it subtracts the gas compensation and then the borrowing fee
   */
  static async getOpenTroveUSDDAmount(contracts, totalDebt) {
  ...
  static async getAmountWithBorrowingFee(contracts, usddAmount) {
    const fee = await contracts.troveManager.getBorrowingFee(usddAmount);
    return usddAmount.add(fee);
  }
  ...
  static getEmittedRedemptionValues(redemptionTx) {
    for (let i = 0; i < redemptionTx.logs.length; i++) {
      if (redemptionTx.logs[i].event === "Redemption") {
        const USDDAmount = redemptionTx.logs[i].args[0];
        const totalUSDDRedeemed = redemptionTx.logs[i].args[1];
        const totalETHDrawn = redemptionTx.logs[i].args[2];
        const ETHFee = redemptionTx.logs[i].args[3];

        return [USDDAmount, totalUSDDRedeemed, totalETHDrawn, ETHFee];
      }
    }
  ...
  static getEmittedLiquidationValues(liquidationTx) {
    for (let i = 0; i < liquidationTx.logs.length; i++) {
      if (liquidationTx.logs[i].event === "Liquidation") {
        ...
        const usddGasComp = liquidationTx.logs[i].args[3];

        return [liquidatedDebt, liquidatedColl, collGasComp, usddGasComp];
      }
    }
  ...
  static getUSDDFeeFromUSDDBorrowingEvent(tx) {
    for (let i = 0; i < tx.logs.length; i++) {
      if (tx.logs[i].event === "USDDBorrowingFeePaid") {
        return tx.logs[i].args[1].toString();
      }
    }
    throw "The transaction logs do not contain an USDDBorrowingFeePaid event";
  }
  ...
  static async getEntireCollAndDebt(contracts, account) {
    ...
    const pendingETHReward = await contracts.troveManager.getPendingCollReward(account);
    const pendingUSDDDebtReward = await contracts.troveManager.getPendingUSDDDebtReward(account);
    const entireColl = rawColl.add(pendingETHReward);
    const entireDebt = rawDebt.add(pendingUSDDDebtReward);
  ...
  static async getCollAndDebtFromWithdrawUSDD(contracts, account, amount) {
  ...
  static async getCollAndDebtFromRepayUSDD(contracts, account, amount) {
    const { entireColl, entireDebt } = await this.getEntireCollAndDebt(contracts, account);
  ...
  static async getCollAndDebtFromAdjustment(contracts, account, ETHChange, USDDChange) {
    const { entireColl, entireDebt } = await this.getEntireCollAndDebt(contracts, account);

    // const coll = (await contracts.troveManager.Troves(account))[1]
    // const debt = (await contracts.troveManager.Troves(account))[0]

    const fee = USDDChange.gt(this.toBN("0"))
      ? await contracts.troveManager.getBorrowingFee(USDDChange)
      : this.toBN("0");
    const newColl = entireColl.add(ETHChange);
    const newDebt = entireDebt.add(USDDChange).add(fee);
  ...
  static async openTrove_allAccounts(accounts, contracts, ETHAmount, USDDAmount) {
    const gasCostList = [];
    const totalDebt = await this.getOpenTroveTotalDebt(contracts, USDDAmount);
  ...
  static async openTrove_allAccounts_randomETH(minETH, maxETH, accounts, contracts, USDDAmount) {
    const gasCostList = [];
    const totalDebt = await this.getOpenTroveTotalDebt(contracts, USDDAmount);
  ...
  static async openTrove_allAccounts_randomETH_ProportionalUSDD(
    minETH,
    maxETH,
    accounts,
    contracts,
    ...
    for (const account of accounts) {
      const randCollAmount = this.randAmountInWei(minETH, maxETH);
      const proportionalUSDD = web3.utils.toBN(proportion).mul(web3.utils.toBN(randCollAmount));
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, proportionalUSDD);
  ...
  static async openTrove_allAccounts_randomETH_randomUSDD(
    minETH,
    maxETH,
    accounts,
    contracts,
    ...
    for (const account of accounts) {
      ...
      const randUSDDProportion = this.randAmountInWei(minUSDDProportion, maxUSDDProportion);
      const proportionalUSDD = web3.utils
        .toBN(randUSDDProportion)
      ...
      if (logging && tx.receipt.status) {
        i++;
        const ICR = await contracts.troveManager.getCurrentICR(account, price);
        // console.log(`${i}. Trove opened. addr: ${this.squeezeAddr(account)} coll: ${randCollAmount} debt: ${proportionalUSDD} ICR: ${ICR}`)
      }
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
    }
  ...
  static async openTrove_allAccounts_randomUSDD(minUSDD, maxUSDD, accounts, contracts, ETHAmount) {
    const gasCostList = [];

    for (const account of accounts) {
      const randUSDDAmount = this.randAmountInWei(minUSDD, maxUSDD);
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, randUSDDAmount);
      const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(
        contracts,
        ETHAmount,
        totalDebt
  ...
  static async openTrove_allAccounts_decreasingUSDDAmounts(
    accounts,
    contracts,
    ETHAmount,
    maxUSDDAmount
    ...
    let i = 0;
    for (const account of accounts) {
      const USDDAmount = (maxUSDDAmount - i).toString();
      const USDDAmountWei = web3.utils.toWei(USDDAmount, "ether");
      const totalDebt = await this.getOpenTroveTotalDebt(contracts, USDDAmountWei);
      const { upperHint, lowerHint } = await this.getBorrowerOpsListHint(
        contracts,
        ETHAmount,
        totalDebt
  ...
  static async openTrove(
    ...
    if (!maxFeePercentage) maxFeePercentage = this.toBN(this._100pct);
    if (!extraUSDDAmount) extraUSDDAmount = this.toBN(0);
    else if (typeof extraUSDDAmount == "string") extraUSDDAmount = this.toBN(extraUSDDAmount);
    ...
    if (extraUSDDAmount) {
      usddAmount = usddAmount.add(extraUSDDAmount);
    }
    ...
    if (extraParams.value) {
      ...
      collAmount = extraParams.value;
      delete extraParams.value;
    }

    if (collAmount) {
      let approveAddress = contracts.borrowerOperations.address;

      if (extraParams.from && contracts.borrowerOperations.proxies) {
        approveAddress = contracts.borrowerOperations.proxies[extraParams.from].address;
      }

      let allowance = await contracts.weth.allowance(extraParams.from, approveAddress);

      allowance = allowance.add(collAmount);

      await contracts.weth.approve(approveAddress, allowance, extraParams);
    }

    const _txPromise = () => {
      if (contracts.borrowerOperations.proxies) {
      ...
      return contracts.borrowerOperations.methods[
        "openTrove(uint256,uint256,uint256,address,address)"
      ](maxFeePercentage, collAmount, usddAmount, upperHint, lowerHint, extraParams);
    };
  ...
  static async addColl(contracts, { collAmount, upperHint, lowerHint, extraParams }) {
    if (!upperHint) upperHint = this.ZERO_ADDRESS;
    if (!lowerHint) lowerHint = this.ZERO_ADDRESS;

    if (collAmount) {
      await contracts.weth.deposit({ from: extraParams.from, value: collAmount });
      let allowance = await contracts.weth.allowance(
        extraParams.from,
      ...
      await contracts.weth.approve(contracts.borrowerOperations.address, allowance, extraParams);
    }

    const tx = await contracts.borrowerOperations.methods["addColl(uint256,address,address)"](
      collAmount,
      upperHint,
      lowerHint,
      extraParams
  ...
  static async adjustTrove(
    ...
    if (collDeposited) {
      await contracts.weth.deposit({ from: extraParams.from, value: collDeposited });
      let allowance = await contracts.weth.allowance(
        extraParams.from,
        contracts.borrowerOperations.address
      );

      allowance = allowance.add(collDeposited);

      await contracts.weth.approve(contracts.borrowerOperations.address, allowance, extraParams);
    }

    if (!borrower) {
      return await contracts.borrowerOperations.methods[
    ...
    return await contracts.borrowerOperations.methods[
      "adjustTrove(address,uint256,uint256,uint256,uint256,bool,address,address)"
    ](
      borrower,
      maxFee,
      collDeposited,
  ...
  static async withdrawUSDD(
    contracts,
    { maxFeePercentage, usddAmount, ICR, upperHint, lowerHint, extraParams }
  ) {
    ...
    assert(
      !(usddAmount && ICR) && (usddAmount || ICR),
      "Specify either usdd amount or target ICR, but not both"
    );
    ...
    if (ICR) {
      ...
      usddAmount = await this.getNetBorrowingAmount(contracts, increasedTotalDebt);
    } else {
      increasedTotalDebt = await this.getAmountWithBorrowingFee(contracts, usddAmount);
    }

    await contracts.borrowerOperations.withdrawUSDD(
      maxFeePercentage,
      usddAmount,
      upperHint,
      lowerHint,
  ...
  static async adjustTrove_allAccounts(accounts, contracts, ETHAmount, USDDAmount) {
    const gasCostList = [];
    ...
    for (const account of accounts) {
      ...
      let USDDChangeBN = this.toBN(USDDAmount);
      ...
      let isDebtIncrease = USDDChangeBN.gt(zero);
      USDDChangeBN = USDDChangeBN.abs();
  ...
  static async adjustTrove_allAccounts_randomAmount(
    ...
    for (const account of accounts) {
      ...
      let USDDChangeBN = this.toBN(this.randAmountInWei(USDDMin, USDDMax));
      ...
      const gas = this.gasUsed(tx);
      // console.log(`ETH change: ${ETHChangeBN},  USDDChange: ${USDDChangeBN}, gas: ${gas} `)
  ...
  static async withdrawUSDD_allAccounts(accounts, contracts, amount) {
    const gasCostList = [];

    for (const account of accounts) {
      const { newColl, newDebt } = await this.getCollAndDebtFromWithdrawUSDD(
        contracts,
        account,
        amount
      );
      ...
      const tx = await contracts.borrowerOperations.withdrawUSDD(
        this._100pct,
        amount,
        upperHint,
        lowerHint,
  ...
  static async withdrawUSDD_allAccounts_randomAmount(min, max, accounts, contracts) {
    const gasCostList = [];

    for (const account of accounts) {
      const randUSDDAmount = this.randAmountInWei(min, max);
  ...
  static async repayUSDD_allAccounts(accounts, contracts, amount) {
    const gasCostList = [];

    for (const account of accounts) {
      const { newColl, newDebt } = await this.getCollAndDebtFromRepayUSDD(
        contracts,
        account,
        amount
      );
      ...
      const tx = await contracts.borrowerOperations.repayUSDD(amount, upperHint, lowerHint, {
        from: account
      });
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
  ...
  static async repayUSDD_allAccounts_randomAmount(min, max, accounts, contracts) {
    const gasCostList = [];
    ...
    for (const account of accounts) {
      ...
      const tx = await contracts.borrowerOperations.repayUSDD(randUSDDAmount, upperHint, lowerHint, {
        from: account
      });
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
  ...
  static async redeemCollateral(redeemer, contracts, USDDAmount, maxFee = this._100pct) {
    const price = await contracts.priceFeedTestnet.getPrice();
    const tx = await this.performRedemptionTx(redeemer, price, contracts, USDDAmount, maxFee);
  ...
  static async redeemCollateral_allAccounts_randomAmount(min, max, accounts, contracts) {
    ...
    for (const redeemer of accounts) {
      ...
      await this.performRedemptionTx(redeemer, price, contracts, randUSDDAmount);
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
    }
    return this.getGasMetrics(gasCostList);
  ...
  static async performRedemptionTx(redeemer, price, contracts, USDDAmount, maxFee = 0) {
    const redemptionhint = await contracts.hintHelpers.getRedemptionHints(USDDAmount, price, 0);
    ...
    if (contracts.troveManager.proxies) {
      return await contracts.troveManager.redeemCollateral(
        USDDAmount,
        firstRedemptionHint,
    ...
    return await contracts.troveManager.methods[
  ...
  static async provideToSP_allAccounts_randomAmount(min, max, accounts, stabilityPool) {
    const gasCostList = [];
    for (const account of accounts) {
      const randomUSDDAmount = this.randAmountInWei(min, max);
      const tx = await stabilityPool.provideToSP(randomUSDDAmount, this.ZERO_ADDRESS, {
        from: account
      });
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
  ...
  static async withdrawFromSP_allAccounts_randomAmount(min, max, accounts, stabilityPool) {
    ...
    for (const account of accounts) {
      const randomUSDDAmount = this.randAmountInWei(min, max);
      const tx = await stabilityPool.withdrawFromSP(randomUSDDAmount, { from: account });
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
    }
    return this.getGasMetrics(gasCostList);
  ...
  // --- DEFT & Lockup Contract functions ---
  ...
  static async getTimeFromSystemDeployment(deftToken, web3, timePassedSinceDeployment) {
    const deploymentTime = await deftToken.getDeploymentStartTime();
