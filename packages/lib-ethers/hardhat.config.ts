...
const aatAddresses = {
  mainnet: "0x686c650dbcfeaa75d09b883621ad810f5952bd5d",
  ropsten: "",
  rinkeby: "",
  goerli: "",
  kovan: ""
...
const hasAAT = (network: string): network is keyof typeof aatAddresses => network in aatAddresses;
...
task("deploy", "Deploys the contracts to the network")
  ...
  .setAction(
    ...
    let aatToken: string | undefined = undefined;
    if (hasAAT(env.network.name)) {
      aatToken = aatAddresses[env.network.name];
    } else {
      aatToken = undefined;
    }
